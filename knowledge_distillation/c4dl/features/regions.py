...
def extract_and_save_patches(
    ...
    Examples:
        ...
        >>> result = extract_and_save_patches_grouped(datamap, patch_dict, '0000', 'ctth_alti')
    """
...
def get_patches(
    ...
    try:
        ...
        # print("Continuing try block")

        # this is a dictionary of time -> list of patch coordinates (for each timestamp)
        # wrong_patch_format = [] # check if the patch format is correct
        for idx in range(len(list(patches.items()))): # for (t, p_coord) in patches.items()
            if archive_path is not None:
                # print("Archive path is NOT NONE")
                data = reader.variable_for_time(list(patches.items())[idx][0], variable, archive_path[idx]) # t
            else:
                # print("Archive path is NONE")
                data = reader.variable_for_time(list(patches.items())[idx][0], variable) # t
            ...
            # print(f"Number of non 0 values in data AFTER post-processing: {np.count_nonzero(data)}")
            # print(list(patches.items())[idx][1])
            # print(len(list(patches.items())[idx][1])) # we got to this point
            time_sec = np.int64(
                (pd.Timestamp(list(patches.items())[idx][0]).to_pydatetime()-epoch).total_seconds()
            ) # t
            
            for (pi, pj, pi_end, pj_end) in list(patches.items())[idx][1][0]: # p_coord
                # pi, pj, pi_end, pj_end = index values for the patch
                # patch_box HAS TO BE 32x32
                # We have to standardize the patch size to patch_shape

                # INITIAL CODE (USED FOR DEBUGGING)
                # patch_box = data[
                #     pi*patch_shape[0]:(pi+1)*patch_shape[0],
                #     pj*patch_shape[1]:(pj+1)*patch_shape[1],
                # ].copy()

                # print((pi, pj, pi_end, pj_end))
                if pi == pi_end or pj == pj_end:
                    # wrong_patch_format.append((pi, pj, pi_end, pj_end))
                    continue

                # Extract patch using the coordinates
                ...
                if len(data.shape) == 3:
                    ...
                    # If data has 4 dimensions, we remove the first dimension
                    patch_box = np.squeeze(data, axis=0)
                    # print(f"Patch box shape after squeeze: {patch_box.shape}")

                    # Move channel axis to the end
                    # patch_box = np.moveaxis(patch_box, 0, -1)  
                    # Alternatively, use np.transpose
                    patch_box = patch_box.transpose((1, 2, 0))  # (height, width, channels)
                    # print(f"Patch box shape after transpose: {patch_box.shape}")
                    
                    # Global average on the last axis (channels)
                    patch_box = np.mean(patch_box, axis=-1)[
                        int(pi):int(pi_end),
                        int(pj):int(pj_end)
                    ].copy()  # average across channels
                    # print(f"Patch box shape after averaging: {patch_box.shape}")
                ...
                ################################ OLD CODE ################################
                # # Resize to 32x32 using numpy interpolation
                # patch_box = resize_patch(patch_box, (32, 32))

                # if nonzero_count_func(patch_box) > 0 or nonzero_count_func is None:
                #     print(f"Nonzero count function (patch_box): {nonzero_count_func(patch_box)}")
                #     print(f"Nonzero count function: {nonzero_count_func}")
        ...
        # if len(wrong_patch_format) > 0:
        #     print(f"Wrong patch format length: {len(wrong_patch_format)}")
    ...
    print(f"Patch data done for variable {variable}")
...
def box_locations(patch_coords, patch_times, 
    ...
    # print("Box location array size:", (t_size,i1,j1))
    ...
    for (t,(i,j)) in times_coords:
        tc = (t-t0) // dt
        loc[tc,i,j] = True
    # Until now a 3D boolean array was created with all patch locations

    (dtc,di,dj) = box_size
    kernel = np.ones((di,dj), dtype=np.uint32)
    kernel_size = di*dj
    
    # Create a dictionary to hold box locations
    box_locs = {}
    ...
    for (tc0, lt) in enumerate(loc):
        ...
        # lt = location data with True and False, kernel = 8x8 matrix of True
        f = convolve(lt, kernel, mode='valid', method='direct')
        # verification if all values in one sample location are True
        candidates = (f == kernel_size) 
        # print(f"Candidates {candidates}")
        
        # extract only the True value coords (numbers) from the meshgrid
        i_list = gi[candidates]
        if len(i_list) == 0:
            continue

        j_list = gj[candidates]
        # print(f"i_list {i_list}, j_list {j_list}")

        for (i0,j0) in zip(i_list,j_list):
            i1 = i0 + di
            j1 = j0 + dj

            # Can access these indices due to gi, gj meshgrids
            box = loc[tc0:tc1,i0:i1,j0:j1] 
            assert(box.shape[1]==di)
            assert(box.shape[2]==dj)
            if box.all(): # select the box with only True values
