...
class BatchGenerator:
    def __init__(self, 
        ...
        # indices for retrieving source data for RZC
        self.raw_batch_index = {}
        self.setup_batch_index(primary_raw_var, raw[primary_raw_var], box_size)
        print(f"Raw batch index for primary variable '{primary_raw_var}': {self.raw_batch_index[primary_raw_var]}")
        index_limits = self.raw_batch_index[primary_raw_var].index_limits
        print(f"Index limits: {index_limits}")
        ...
        # box_locs converted to numpy arrays
        self.coords_by_time = {
        ...
        print(f"Fixed coordinates: {self.fixed_coord}")

        # generate a random number used to
        # select a random coordinate for each time in the validation and test set
        for t in fixed_coord_times:
            coords = self.coords_by_time[t]
            # print(coords)
            # print(len(coords))
            # print(coords_by_time)
            # exit(0)
            self.fixed_coord[t] = self.rng.randint(len(coords))
        print(f"Fixed coordinates after selection: {self.fixed_coord}")
        # exit(0)
    ...
    for k in prange(t0_all.shape[0]):
        ...
        for t in range(t0, t1):
            ...
            for i in range(i0, i1):
                bi0 = int((i-i0) * bi_size)
                bi1 = int(bi0 + bi_size)
                for j in range(j0, j1):
                    ind = int(patch_index[tt,i,j])
                    bj0 = int((j-j0) * bj_size)
                    bj1 = int(bj0 + bj_size)
                    if ind >= 0:
