...
- **Separat** — `--max_batches N` rulează o epocă pe N loturi per partiție și salvează, o probă a configurației; `--batch_size N` suprascrie configurația. Pista de fulgere folosește `swin_unet`; pista de precipitații se antrenează mai întâi cu `swin_unet`, ca referință, apoi cu `swin_unet_cvae`, iar cele două se compară pe clasele intense la orizonturile lungi.
