...
- **Notă** — artefactele `best` și `_latest` ale unui model sunt tratate ca un singur model, sufixul `_latest` fiind ignorat la potrivire, `best` având prioritate când există ambele. `--include_baseline` are efect numai pentru categoria precipitațiilor: nu există încă un model de referință pentru fulgere. `--models`, `--label`, `--hit_levels` și `--seasons` restrâng sau redenumesc ceea ce se reprezintă.
