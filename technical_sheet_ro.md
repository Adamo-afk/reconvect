...
- **Scrie** — `models/coalition_<run_tag>_finetuned.keras` (ponderile; modelul este reconstruit din baza și din istoric la încărcare), `models/history_<run_tag>_finetuned.json` cu hiperparametrii capului și, per epocă, termenii funcției de cost, CSI / FSS3 / FSS9 per orizont, Brier și CSI-ul persistenței (fulgere), fracția adiacentă (precipitații), raportul corecției și `s_l` per orizont, norma gradientului la nivelul 2 și, pentru cVAE, KL, unitățile active, statisticile porții, plafonul posterior și metricile de ansamblu; checkpoint-ul per epocă sub `models/checkpoints/`.
