...
| `selection_rule` | `pixel` | `--rule pixel\|box` | `pixel`: un patch care conține ≥ 1 pixel de cluster; `box`: un patch care intersectează reuniunea chenarelor de 256 × 256 centrate pe centroizii clusterelor. Înregistrată în index; regula înregistrată este cea implicită |
...
- **Regulă** — `--rule pixel|box`, înregistrată în index ca `selection_rule` și implicită odată ce indexul există. `pixel` selectează un patch care conține ≥ 1 pixel de cluster — toate indexurile de până acum. `box` centrează un chenar de 256 × 256 pe centroidul fiecărui cluster și selectează un patch care intersectează reuniunea acestor chenare: fiecare celulă își păstrează 128 px de context de fiecare parte, iar o celulă de la marginea unui patch îl aduce și pe vecinul uscat. Schimbarea regulii reconstruiește un index diferit și invalidează rezerva de patch-uri.
- **Scrie** — `our_data/patch_index/patch_index.csv` și `.json` **CRITIC**
- **Grafic** — cu `--date --plot`, câte un GIF per diagnostic și per zi, cu un cadru per pas de timp: `plots/dbscan_<rule>_patch_selection/<date>.gif` (stânga: câmpul peste prag, centroidul fiecărui cluster și un chenar de 256 × 256 în jurul său; dreapta: masca testată de regula în vigoare — pixelii clusterelor sub `pixel`, reuniunea chenarelor centrate pe centroizi, cu pixelii clusterelor desenați deasupra, sub `box` — grila 6 × 3 punctată în roșu, iar patch-urile care conțin ≥ 1 pixel din mască conturate în verde) și `plots/patch_highlight/<date>.gif` (câmpul și selecția, cu frontiere). Un fișier NetCDF echivalent per pas activ, ~28 MB fiecare, este scris în `plots/nc/`. Exclusiv pentru diagnoză; `--purge_plots` le elimină.
- **Baleiaj** — `--sweep [MMH…] --sweep_stride N` (doar vizualizare) grupează fiecare al N-lea pas de timp din interval la fiecare prag și desenează `plots/threshold_sweep.png`: proporția pașilor de timp cu cel puțin un patch selectat și numărul de patch-uri per eșantion activ în funcție de prag, pragul în vigoare marcat cu roșu; `threshold_sweep.csv` conține valorile. Indexul nu este modificat.
...
```bash
python train_models.py --config training.config --mode mtg_lightning_opera_occurrence --stage finetune --base_checkpoint models/coalition_<run_tag>.keras --period f34 --head swin_unet
python train_models.py --config training.config --mode mtg_lightning_opera_rainfall --stage finetune --base_checkpoint models/coalition_<run_tag>.keras --period f34 --head swin_unet
python train_models.py --config training.config --mode mtg_lightning_opera_rainfall --stage finetune --base_checkpoint models/coalition_<run_tag>.keras --period f34 --head swin_unet_cvae
```
- **Descriere** — îngheață modelul de bază și antrenează o corecție Swin-UNet pe două niveluri peste trăsăturile decodorului și logiții acestuia: `logits = s_l · L_b + head(x)`, capetele inițializate la zero, astfel încât pasul 0 este modelul înghețat (verificat la pornire). Nivelul 1 la 1 km (d 64, 4 capete), nivelul 2 la 2 km (d 128, 8 capete), câte 2 blocuri, ferestre 8 × 8 cu deplasarea de poziție relativă și masca de deplasare, supraeșantionare biliniară cu legătura de la nivelul 1, expandare prin pixel shuffle; ultimele două cadre ale câmpului-țintă și ale grupului MR intră prin tulpini brute. `swin_unet_cvae` (precipitații) adaugă un VAE condiționat: prior `p(z | c)`, posterior `q(z | c, y)`, latent `16² × 32`, FiLM cu poartă pe nivelul 2, funcția de cost `+ beta · mean max(KL, free bits)` cu `beta` încălzit treptat.
- **Funcția de cost** — fulgere: BCE cu `pos_weight = min(N_neg/N_pos, 30)` (varianta focală ca rezervă, `lightning_gamma`); precipitații determinist: CE focală ponderată + 0,2 · L1 pe clasa așteptată; precipitații cVAE: CE ponderată pe clase (fără factor focal) + termenul KL. AdamW lr 2e-4, wd 0,05 fără normări, bias-uri, tabele de poziție și `s_l`; cosinus cu 1 epocă de încălzire; tăiere 1,0; stochastic depth 0,1; 18 epoci, oprire timpurie pe `val_csi_mid` (t+30, t+45), răbdare 4.
- **Scrie** — `models/coalition_<run_tag>_finetuned.keras` (ponderile; modelul este reconstruit din baza și din istoric la încărcare; `_finetuned_cvae` pentru capul cVAE, astfel încât ambele capete pentru precipitații coexistă), `models/history_<run_tag>_finetuned[_cvae].json` cu hiperparametrii capului și, per epocă, termenii funcției de cost, CSI / FSS3 / FSS9 per orizont, Brier și CSI-ul persistenței (fulgere), fracția adiacentă (precipitații), raportul corecției și `s_l` per orizont, norma gradientului la nivelul 2 și, pentru cVAE, KL, unitățile active, statisticile porții, plafonul posterior și metricile de ansamblu; checkpoint-ul per epocă sub `models/checkpoints/`.
- **Separat** — `--max_batches N --epochs E` rulează E epoci pe N loturi per partiție și salvează, o probă a configurației; `--batch_size N` și `--epochs E` suprascriu configurația. Pista de fulgere folosește `swin_unet`; pista de precipitații se antrenează mai întâi cu `swin_unet`, ca referință, apoi cu `swin_unet_cvae`, iar cele două se compară pe clasele intense la orizonturile lungi.
...
- **Scrie** — `validation/<stem>_summary.json` (`<stem>` = `rainfall_<domeniu>_<tag>`; figurile execuției merg în `validation/<stem>/`) **CRITIC** — pragurile calibrate și blocul `per_patch`. `<tag>` este eticheta de artefact a modelului, astfel încât mai multe modele validate pe același domeniu pot coexista; `generate_report` primește `--rainfall_tag` atunci când există mai multe.
- **Scrie** — `…_samples.csv`, cu `csi_t+<k>` și procentele de detecții / ratări / alarme false per orizont la pragul HIGH calibrat.
- **Separat** — un cap cVAE pentru precipitații (`mode:cvae`; `--cvae` la evaluare, inferență și vizualizare) este evaluat ca o compoziție a `--members` extrageri ale latentului (implicit 3): pragurile calibrate pe media priorului, fiecare membru evaluat per petec pe partiția de validare, cel mai bun membru păstrat per petec, domeniul evaluat pe această compoziție; `…_members.png` arată câte un panou per membru cu CSI-ul său per petec, petecele câștigate în verde, iar blocul `members` din rezumat este cel cu care vizualizatorul și inferența compun. `--baseline` validează SepConv-ens post-procesat în aceleași clase (fără histerezis); `--max_samples N` limitează o execuție de probă; `--weights latest` evaluează checkpoint-ul per epocă în locul salvării finale (eticheta primește sufixul `_latest`). Același indicator există la A4 și A6.
- **Calibrare** — pe eșantioane extrase din partiția de validare, niciodată pe cele evaluate. Precipitații: faza 1 alege LOW per orizont printr-un baleiaj cu prag simplu pe p(argmax) (`--rainfall_low_threshold` îl fixează în loc); faza 2 alege perechea (LOW, HIGH) per orizont dintre ferestre de lățime `--rainfall_window` așezate una lângă alta de ambele părți ale acelui LOW până la `--rainfall_reach`; domeniul este evaluat la perechea aleasă. `--tune_leads first` calibrează doar pe t+1 și refolosește rezultatul pentru toate orizonturile (ambele categorii). Fulgere: LOW este pragul calibrat de evaluare (citit din `evaluation/eval_<tag>/`, `--lightning_low_threshold` îl suprascrie) sau, cu `--lightning_tune_low`, o fază 1 îl baleiază per orizont pe partiția de validare la fel ca pista de precipitații (`…_tuning_low.png`); HIGH este baleiat deasupra LOW-ului fiecărui orizont. Rezumatul reține `low_threshold_per_lead` și `low_source`, iar inferența și vizualizatorul aplică LOW-ul per orizont prin `--validation_summary`. `--max_samples` extrage ambele seturi reproductibil per lună (`--month_batch`, `--seed`).
- **Populația** — extracția prezice înregistrările partiției din TFRecord-uri (`--datasets_root`), la fel ca evaluarea, și așază petecele selectate de DBSCAN ale fiecărei referințe înapoi pe suprafață, unde rulează histerezisul față de etichetele înregistrărilor. Pixelii din afara acelor petece nu sunt evaluați: validarea măsoară modelul pe petecele de furtună din care a fost construit setul de date, aceeași populație ca evaluarea, nu pe restul uscat al domeniului. Rezumatele de dinaintea acestei schimbări evaluau întreaga suprafață și nu sunt comparabile.
- **Cost** — o singură trecere prin înregistrări per fază, circa 5.300 de petece per partiție pentru f34, fără citirea câmpurilor reproiectate; maximele OPERA de care are nevoie selecția eșantioanelor sunt reținute în `our_data/data_statistics/opera_rainfall_max_cache.json`; faza 2 a pistei de precipitații reia din memorie predicțiile fazei 1 (`--cache_gb`, implicit o treime din memoria fizică) în loc să treacă din nou eșantioanele prin model. Niciuna dintre acestea nu schimbă vreun număr.
- **Populația** — ca pe pista de precipitații: înregistrările partiției, prezise pe loturi din TFRecord-uri (`--datasets_root`, `--batch_size`), studentul din înregistrările profesorului cu canalele HR decupate, petecele fiecărei referințe așezate înapoi pe suprafață pentru histerezis față de etichetele înregistrărilor. Probabilitățile sunt ieșirea proprie a fiecărui petec; amestecul Hann peste pozițiile suprapuse aparține inferenței și vizualizării pe zile, nu extracției.
- **Cost** — o singură etichetare a componentelor conexe per eșantion și orizont deservește toți candidații HIGH (rezultate identice cu pragul aplicat fiecăruia); o singură trecere prin înregistrări per fază.
- **Grafic** — `…_metrics.png` (barele FAR/POD/CSI), `…_coverage.png` (IoU în raport cu suprapunerea ponderată pe clase, precipitații), `…_hmf.png` (detecții / ratări / alarme false per eșantion, marcator per orizont, linie roșie la 50 %, primul / medianul / ultimul eșantion datate pe axa x), `…_hmf_percentiles.png` (per orizont p10–p90 / p25–p75 / mediana acestor rate și proporția eșantioanelor peste și sub 50 %), `…_hysteresis.png` (trei grafice ale ratelor de detecții / ratări / alarme false, detecțiile și ratările față de pixelii activi în adevărul de teren, alarmele false față de pixelii activi în predicție: per orizont ratele deciziei brute și ale celei post-procesate; per orizont diferența post-procesat minus brut a fiecărei rate; și variația fiecărei rate față de primul orizont pentru brut și post-procesat; brut înseamnă argmax pentru precipitații și p ≥ LOW pentru fulgere), `…_tuning_low.png` și `…_tuning_high.png` (precipitații: baleiajul din faza 1 și ferestrele din faza 2 per orizont) sau `…_tuning.png` (fulgere: baleiajul HIGH per orizont). Toate sunt generate din rezumatul și CSV-ul salvate; `python validate_predictions.py --plots <summary.json>` le regenerează pentru oricare categorie. Suprapuneri pe zile cu `--date`
...
```
- **Descriere** — antrenează cele trei modele de bază ale modelului de referință, precum și ablația pe arhitectura RECONVECT. Fiecare `Bm{k}` este denumit după **propriul** său orizont — cât de departe estimează față de finalul propriei ferestre de intrare — nu după orizontul pe care îl deservește: Bm1 furnizează t+1, Bm3 furnizează t+2 și t+3, iar Bm5 furnizează t+4, deoarece fereastra sa este decalată cu un pas în urmă. **Orizontul de prognoză este t+4 = 60 min**; cifra din denumirea unui model de bază este orizontul său propriu în pași, niciodată un orizont în minute.
...
- **Grafic** — `metrics_per_leadtime.png`; `--plot_samples N` reprezintă clasa observată față de cea estimată și valorile mm/h estimate. Evaluarea pentru fulgere adaugă `threshold_sweep.png`, CSI cumulat în funcție de pragul de decizie pe partiția de validare (unde este ales) și pe cea de test, cu punctul de operare și optimul de test marcate; acel prag este LOW-ul de histerezis al validării pentru fulgere.
...
```
- **Descriere** — citește `evaluation_results.json` și fișierele de validare `_samples.csv` etichetate ale fiecărui model dintr-o categorie și le reprezintă împreună: curbele metricilor per orizont de prognoză (o singură figură-grilă, un panou per metrică, fiecare cu direcția mai bună în titlu), un tabel în stil de articol științific cu cea mai bună valoare evidențiată și proporția eșantioanelor al căror procent de detecție (precipitații: pixelii activi din adevărul de teren detectați la pragul HIGH calibrat; fulgere: POD per eșantion) depășește 50…90 % per orizont, per anotimp și pe întreaga fereastră.
- **Scrie** — `comparison/<track>/metrics_per_leadtime.png`, `comparison_table.{csv,tex,png}`, `hit_levels_<domeniu>/hit_levels.png`, `hit_levels_<domeniu>/hit_levels.csv`, `comparison_summary.json` (livrabile; nu sunt citite de alt script). `<domeniu>` urmează `--split` / `--year --month`: `test`, `<Y>_<M>`, `test_<Y>_<M>` sau `all_months` atunci când toate execuțiile pe luni întregi sunt cumulate.
- **Notă** — `--models TAG…` numește artefacte exacte (o etichetă simplă = salvarea finală, o etichetă `_latest` = rularea din checkpoint); `--weights best|latest|any` alege altfel ce stare salvată a fiecărui model este citită; artefactele `best` și `_latest` ale unui model sunt un singur model, iar `any` (implicit) ia `best` când există, altfel `latest`. `--include_baseline` are efect numai pentru categoria precipitațiilor: nu există încă un model de referință pentru fulgere. `--models`, `--label`, `--hit_levels` și `--seasons` restrâng sau redenumesc ceea ce se reprezintă.
...
- **Scrie** — `validation/<stem>_summary.json` cu `per_patch` **CRITIC**
...
- **Rata de învățare** — același program cosinus cu încălzire ca modelele de bază, citit din `[lr_schedule]` din `training.config` (`--config`; `--learning_rate` suprascrie `initial_lr`); fără reducere pe platou peste el.
