"""
Lightning activity by time of day - reads `lightning_active_steps.csv`
produced by `summarize_lightning_data.py` and emits one bar chart.
...
For each HH:MM across all dates, count how many days have activity at
that time step: the lightning diurnal cycle. Per-date coverage is in
`lightning_summary.csv` and `lightning_coverage.png`; this is the one
question only the per-(date, HH:MM) index can answer.
...
def main():
    parser = argparse.ArgumentParser(
        description="Lightning activity by time of day, from "
    ...
    print("=" * 70)
    print("Lightning Activity by Time of Day")
    print("=" * 70)
    ...
    print(f"  {n_dates} dates, {len(times)} unique time steps")
