...
class MSGRadianceCCS4Reader(DatasetReader):
    ...
    def read_fields_from_archive(self, time):
    ...
    def data_for_time(self, time):
        if time not in self.cache:
            self.cache[time] = self.read_fields_from_archive(time)
...
class NWCSAFCCS4Reader(DatasetReader):
    ...
    def read_fields_from_archive(self, time, variables=None):
    ...
    def get_scale(self, time, variable):
        ...
        #print(scale_factor, add_offset, valid_range)
