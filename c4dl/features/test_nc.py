...
from check_patch_extraction import plot_patches_on_blank
...
def process_netcdf_file(file_path, lat, lon, method='nearest', base_output_dir=None):
    ...
    if data_type == 'radar_data':
        with Dataset(file_path, 'r') as ds:
            ...
            regridded = np.flipud(regridded)  # Flip around the Ecuator
            # =====================================================================
...
def extract_patches(datamap, centers, patch_size=256, stride=32):
    ...
    if centers:
        for center in centers:
            ...
            if (center[0] >= half_size and center[0] <= datamap.shape[0] - half_size and
                ...
                # print(f"Row end - start = {row_end - row_start}")
                # print(f"Col end - start = {col_end - col_start}")
                ...
                for i in range(upper_left[0], upper_left[0] + 225, stride):
                    for j in range(upper_left[1], upper_left[1] + 225, stride):
                        ul_small_patch_coords.append((i, j))
                        lr_small_patch_coords.append((i+stride, j+stride))
...
def plot_clusters_and_patches_with_flip_control(datamap, centers, patch_size=256, 
    ...
    """
    Plot rainfall data with cluster centers and patch boundaries.
    
    Converts datamap to binary (threshold > 10), optionally flips the data,
    and visualizes cluster centers with surrounding patch boundaries.
    
    Parameters
    ----------
    datamap : np.ndarray
        2D array of rainfall data
    centers : list or np.ndarray
        Cluster center coordinates as (row, col) pairs
    patch_size : int, default=256
        Size of square patches to draw around centers
    flip_horizontal : bool, default=False
        Flip data left-right
    flip_vertical : bool, default=False
        Flip data top-bottom
    ...
    # Display the processed datamap
    im = ax.imshow(processed_datamap, cmap='gray', aspect='equal')
    
    # Add cluster centers
    ax.scatter(processed_centers[:, 1], processed_centers[:, 0],
              marker='x', s=200, c='red', linewidths=3,
              label='Cluster Centers', zorder=3)
    
    # Add patch boundaries (256×256 rectangles around each center)
    for center in processed_centers:
        x0 = center[1] - half_size  # column - half_size
        y0 = center[0] - half_size  # row - half_size
        
        # Rectangle: (x, y) is bottom-left corner with default origin
        rect = Rectangle(
            (x0, y0),
            patch_size,  # width
            patch_size,  # height
    ...
    ax.set_title('Rainfall Regions with Cluster Centers and Patch Boundaries', 
                 fontsize=14)
...
def variable_patch_comparison(all_default_vars, args):
    """
...
# def area_with_country_boundaries():
...
#     # Create figure with Stereographic projection centered on Romania
#     projection = ccrs.Stereographic(central_longitude=25.0, central_latitude=46.0)
    
#     fig, ax = plt.subplots(figsize=(12, 10), 
#                           subplot_kw={'projection': projection})

#     # Get bounds from your lat/lon grids
...
#     ax.add_feature(cfeature.BORDERS, linewidth=2, edgecolor='red')
    
#     # Load admin level 1 (counties) polygons
#     states_provinces = cfeature.NaturalEarthFeature(
#         category='cultural',
#         name='admin_1_states_provinces',
#         scale='10m', # changes resolution to be able to see county boundaries
#         facecolor='none'
#     )
    
#     # Get the shapefile reader
#     shpfilename = shpreader.natural_earth(resolution='10m',
#                                           category='cultural',
#                                           name='admin_1_states_provinces')
#     reader = shpreader.Reader(shpfilename)
    
#     # Counties to highlight (you can add more)
#     highlight_counties = {
#         'Arad': 'green',    
#     }
    
#     # Iterate through all admin regions
#     for record in reader.records():
#         county_name = record.attributes.get('name', '')
        
#         # Check if this county should be highlighted
#         if county_name in highlight_counties:
#             ax.add_geometries([record.geometry], ccrs.PlateCarree(),
#                             facecolor=highlight_counties[county_name],
#                             alpha=0.5,
#                             edgecolor='darkred',
#                             linewidth=1)
#         else:
#             # Draw other counties with just borders
#             ax.add_geometries([record.geometry], ccrs.PlateCarree(),
#                             facecolor='none',
#                             edgecolor='darkred',
#                             linewidth=0.5)
...
#     # Plot your grid extent as a rectangle
#     rect = Rectangle((lon_min, lat_min), lon_max - lon_min, lat_max - lat_min,
#                      linewidth=3, edgecolor='purple', facecolor='none',
#                      transform=ccrs.PlateCarree())
#     ax.add_patch(rect)

#     plt.title(f'Romania Grid Coverage\nEPSG:31700 - {romania_grid_area["width"]}x{romania_grid_area["height"]} pixels', 
#               fontsize=14, fontweight='bold')
#     plt.tight_layout()
#     plt.savefig('romania_grid_coverage.png', dpi=150, bbox_inches='tight')
#     plt.show()
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help='Target date and time in format YYYY-MM-DD HH:MM'
    )
    parser.add_argument(
        '--regrid',
        action='store_true',
        help='Flag to indicate whether to regrid data to Romania grid'
    )
    parser.add_argument(
        '--dbscan_mask',
        action='store_true',
        help='Flag to indicate plotting of boundries and centers of 256x256 patches'\
        'after DBSCAN extraction (mask_values > 10 mm/h)'
    )
    ...
    # # Plot Romania grid with country boundaries
    # area_with_country_boundaries()
    ...
    if args.precipitation_only:
        ...
        for netcdf_dir in precipitation_paths:
            ...
            for nc_file in os.listdir(netcdf_dir)[:SAMPLE_SIZE]:
                ...
                if nc_file.endswith('.nc'):
                    ...
                    # DATA IS REGRIDDED AND FLIPPED FOR RADAR ONLY
                    if args.regrid:
                        process_netcdf_file(
                            file_path=filename,
                            lat=lats,
                    ...
                    # FINDING CLUSTER CENTERS FOR 256X256 PATCHES
                    # AFTER REGRIDDING THE DATA TO ROMANIA GRID
                    ...
                    # =============== Plot clusters and patches ===============
                    if args.dbscan_mask:
                        _, _ = plot_clusters_and_patches_with_flip_control(
                            datamap,
                            valid_centers,
                            patch_size=256, 
                    ...
                    if patches:
                        ...
                        # data_dict exists for saving data for a single day (we have multiple days)
        ...
        # MODIFY FUNCTION SO IT DOES NOT USE datamap (it uses the last datamap read)
        # MODIFY HOW DATA IS READ ACROSS ALL PRODUCTS (considering multiple days)
        # ================== Check patch extraction validity ==========================
        if args.target_datetime is not None:
            print(f"All RZC data: \n{all_data_dict}")
            for k, v in all_data_dict.items():
                print(f"Timestamp: {k}, Number of patch index lists: {len(v)}")
                for list_idx, patch_indices in enumerate(v):
                    print(f"  Patch list {list_idx + 1}: Number of patches: {len(patch_indices)}")
            plot_patches_on_blank(datamap, all_data_dict, args.target_datetime)
        # ===============================================================================
    ...
    if args.radar:
        ...
        # Get path for radar data (all products, except precipitation - RZC)
    ...
    ############### Satellite data processing ################
        ...
        for netcdf_dir in netcdf_dirs:
            ...
            # We are doing this due to DIFFERENT DATA VARIABLITY 
            # (some timestamps are missing from satellite data)
            ...
            if args.target_datetime is not None:
                print(f"All RZC data: \n{sat_data_dict}")
                for k, v in sat_data_dict.items():
                    print(f"Timestamp: {k}, Number of patch index lists: {len(v)}")
                    for list_idx, patch_indices in enumerate(v):
                        print(f"  Patch list {list_idx + 1}: Number of patches: {len(patch_indices)}")
                plot_patches_on_blank(datamap, sat_data_dict, args.target_datetime)
            # ===============================================================================

        ############### Variable patches comparison ################
        variable_patch_comparison(all_default_vars, args)
    ...
    # time intervals from kml file differ from generated data (kml in Google Earth - 3h = generated data)
        ...
        for netcdf_dir in netcdf_dirs:
            ...
            if args.target_datetime is not None:
                print(f"All RZC data: \n{light_data_dict}")
                for k, v in light_data_dict.items():
                    print(f"Timestamp: {k}, Number of patch index lists: {len(v)}")
                    for list_idx, patch_indices in enumerate(v):
                        print(f"  Patch list {list_idx + 1}: Number of patches: {len(patch_indices)}")
                plot_patches_on_blank(datamap, light_data_dict, args.target_datetime)
            # ===============================================================================
