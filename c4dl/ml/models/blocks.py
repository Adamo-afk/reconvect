...
from c4dl.ml.models.layers import ReflectionPadding2D
...
class ResBlock(Layer):
    def __init__(self, channels, **kwargs):
        ...
        self.channels = channels
        # Store original kwargs before any modifications
        self.original_kwargs = kwargs.copy()
        
        # Extract stride but keep a copy in original_kwargs
        self.stride = kwargs.pop("stride", 1)
        time_dist = kwargs.get("time_dist", False)
    ...
    def get_config(self):
        ...
        # Add all the original kwargs
        config.update(self.original_kwargs)
        return config
...
class GRUResBlock(ResBlock):
    def __init__(self, channels, final_activation='sigmoid', **kwargs):
        ...
        self.final_activation = final_activation  # Store for get_config
        self.final_act = Activation(final_activation)

    def get_config(self):
        config = super().get_config()  # Gets ResBlock's config
        config.update({
