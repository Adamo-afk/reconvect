...
# From: https://github.com/juntang-zhuang/Adabelief-Optimizer/blob/update_0.2.0/pypi_packages/adabelief_tf0.2.1/adabelief_tf/AdaBelief_tf.py
...
class AdaBeliefOptimizer(tf.keras.optimizers.Optimizer):
        ...
        config.update(
            {
                ...
                "beta_1": self.beta_1,
                "beta_2": self.beta_2,
                "decay": self._initial_decay if hasattr(self, '_initial_decay') else 0.0,
    ...
    def _resource_apply_dense(self, grad, var):
        ...
        if self._initial_total_steps > 0:
            total_steps = tf.cast(self.total_steps, var_dtype)
            warmup_steps = total_steps * tf.cast(self.warmup_proportion, var_dtype)
            min_lr = tf.cast(self.min_lr, var_dtype)
        ...
        if self.rectify:
            sma_threshold = tf.cast(self.sma_threshold, var_dtype)
            var_t = tf.where(
                sma_t >= sma_threshold,
