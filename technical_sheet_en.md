...
- **Note** — a model's `best` and `_latest` artefacts are matched as one model, the `_latest` suffix dropped, `best` used when both exist. `--include_baseline` has an effect on the rainfall track only: no lightning baseline exists yet. `--models`, `--label`, `--hit_levels` and `--seasons` narrow or rename what is drawn.
