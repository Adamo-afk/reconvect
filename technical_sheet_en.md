...
| Finetune head | two-level Swin-UNet correction on the frozen backbone (`swin_unet`), with a conditional VAE for rain (`swin_unet_cvae`) | finetune stage |
...
- **Sweep** — `--sweep [MMH…] --sweep_stride N` (visualisation only) clusters every Nth timestep of the range at each threshold and draws `plots/threshold_sweep.png`: the share of timesteps with at least one selected patch and the patches per active sample against the threshold, the threshold in force marked in red; `threshold_sweep.csv` holds the numbers. The index is not touched.
...
```
- **Does** — freezes the base model and trains a two-level Swin-UNet correction on its decoder features and logits: `logits = s_l · L_b + head(x)`, heads zero-initialised, so step 0 is the frozen model (checked at start). Level 1 at 1 km (d 64, 4 heads), level 2 at 2 km (d 128, 8 heads), 2 blocks each, 8 × 8 windows with the relative position bias and the shift mask, a bilinear upsample with the level-1 skip, a pixel-shuffle expand; the last two frames of the target field and of the MR group enter through raw stems. `swin_unet_cvae` (rain) adds a conditional VAE: prior `p(z | c)`, posterior `q(z | c, y)`, latent `16² × 32`, gated FiLM on level 2, loss `+ beta · mean max(KL, free bits)` with `beta` warmed up.
- **Loss** — lightning: BCE with `pos_weight = min(N_neg/N_pos, 30)` (focal fallback `lightning_gamma`); rain deterministic: weighted focal CE + 0.2 · expected-class L1; rain cVAE: class-weighted CE (no focal) + the KL term. AdamW lr 2e-4, wd 0.05 off norms, biases, position tables and `s_l`; cosine with 1 warm-up epoch; clip 1.0; stochastic depth 0.1; 18 epochs, early stopping on `val_csi_mid` (t+30, t+45), patience 4.
- **Writes** — `models/coalition_<run_tag>_finetuned.keras` (the weights; rebuilt from the base and the history at load; `_finetuned_cvae` for the cVAE head, so both rain heads coexist), `models/history_<run_tag>_finetuned[_cvae].json` with the head hyperparameters and, per epoch, the loss terms, CSI / FSS3 / FSS9 per lead, Brier and persistence CSI (lightning), adjacent fraction (rain), the correction ratio and `s_l` per lead, the stage-2 gradient norm, and for the cVAE the KL, active units, gate statistics, the posterior ceiling and the ensemble metrics; the per-epoch checkpoint under `models/checkpoints/`.
- **Alone** — `--max_batches N --epochs E` runs E epochs on N batches per split and saves, a dry run of the configuration; `--batch_size N` and `--epochs E` override the config. The lightning track uses `swin_unet`; the rain track is trained with `swin_unet` first, as the baseline, then with `swin_unet_cvae`, and the two are compared on the heavy classes at the long leads.
...
- **Alone** — a cVAE rain head (`mode:cvae`; `--cvae` on the evaluation, the inference and the visualiser) is scored as a composite of `--members` draws of the latent (default 3): thresholds tuned on the prior mean, every member scored per patch on the validation split, the best member per patch kept, the scope scored on that composition; `…_members.png` shows one panel per member with its per-patch CSI, the won patches in green, and the summary's `members` block is what the visualiser and the inference compose with. `--baseline` validates SepConv-ens post-processed into the same classes (no hysteresis); `--max_samples N` caps a trial run; `--weights latest` scores the per-epoch checkpoint instead of the final save (tag suffixed `_latest`). The same flag exists on A4 and A6.
- **Tuning** — on samples drawn from the validation split, never on the scored ones. Rainfall: phase 1 picks LOW per lead with a plain-threshold sweep on p(argmax) (`--rainfall_low_threshold` fixes it instead); phase 2 picks the (LOW, HIGH) pair per lead among windows of `--rainfall_window` tiled on both sides of that LOW out to `--rainfall_reach`; the scope is scored at the pair. `--tune_leads first` tunes on t+1 alone and reuses the result for every lead (both tracks). Lightning: LOW is the evaluation's tuned threshold (read from `evaluation/eval_<tag>/`, `--lightning_low_threshold` overrides), or with `--lightning_tune_low` a phase 1 sweeps it per lead on the validation split as the rainfall track does (`…_tuning_low.png`); HIGH is swept above each lead's LOW per lead. The summary records `low_threshold_per_lead` and `low_source`, and the inference and the visualiser apply the per-lead LOW through `--validation_summary`. `--max_samples` draws both sets reproducibly per month (`--month_batch`, `--seed`).
...
- **Graph** — `…_metrics.png` (FAR/POD/CSI bars), `…_coverage.png` (IoU against class-weighted overlap, rainfall), `…_hmf.png` (per-sample hits / misses / false alarms, marker per lead, red line at 50 %, first / median / last sample dated on the x axis), `…_hmf_percentiles.png` (per lead the p10–p90 / p25–p75 / median of those rates and the share of samples above and below 50 %), `…_hysteresis.png` (three plots of the hit / miss / false-alarm rates, hits and misses over the GT-active pixels, false alarms over the predicted-active pixels: per lead the rates of the raw decision and of the post-processed one; per lead the difference post-processed minus raw of each rate; and the change of each rate against the first lead for raw and post-processed; raw is argmax for rainfall and p ≥ LOW for lightning), `…_tuning_low.png` and `…_tuning_high.png` (rainfall: the phase-1 sweep and the phase-2 windows per lead) or `…_tuning.png` (lightning: the HIGH sweep per lead). All drawn from the saved summary and CSV; `python validate_predictions.py --plots <summary.json>` redraws them for either track. Per-date overlays with `--date`
...
```
- **Does** — reads every model's `evaluation_results.json` and tagged validation `_samples.csv` for one track and draws them together: per-lead metric curves (one grid figure, a panel per metric, each titled with the direction that is better), a paper-style table with the best value in bold, and the share of samples whose hit percentage (rainfall: GT-active pixels detected at the tuned HIGH; lightning: per-sample POD) clears 50…90 % per lead, per season and over the whole window.
- **Writes** — `comparison/<track>/metrics_per_leadtime.png`, `comparison_table.{csv,tex,png}`, `hit_levels_<scope>/hit_levels.png`, `hit_levels_<scope>/hit_levels.csv`, `comparison_summary.json` (deliverables; nothing reads them). `<scope>` follows `--split` / `--year --month`: `test`, `<Y>_<M>`, `test_<Y>_<M>`, or `all_months` when every whole-month run is pooled.
- **Note** — `--models TAG…` names exact artefacts (a plain tag the final save, a `_latest` tag the checkpoint run); `--weights best|latest|any` picks which saved state of each model is read otherwise; a model's `best` and `_latest` artefacts are one model, and `any` (default) takes `best` when it exists, else `latest`. `--include_baseline` has an effect on the rainfall track only: no lightning baseline exists yet. `--models`, `--label`, `--hit_levels` and `--seasons` narrow or rename what is drawn.
