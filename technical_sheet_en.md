...
- **Writes** — `comparison/<track>/metrics_per_leadtime_<metric>.png`, `comparison_table.{csv,tex,png}`, `hit_levels_<scope>/hit_levels.png`, `hit_levels_<scope>/hit_levels.csv`, `comparison_summary.json` (deliverables; nothing reads them). `<scope>` follows `--split` / `--year --month`: `test`, `<Y>_<M>`, `test_<Y>_<M>`, or `all_months` when every whole-month run is pooled.
