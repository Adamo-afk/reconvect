...
- **Alone** — `--max_batches N` runs one epoch on N batches per split and saves, a dry run of the configuration; `--batch_size N` overrides the config. The lightning track uses `swin_unet`; the rain track is trained with `swin_unet` first, as the baseline, then with `swin_unet_cvae`, and the two are compared on the heavy classes at the long leads.
