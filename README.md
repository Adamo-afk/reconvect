...
**What it buys.** `<stem>_hysteresis.png` compares the raw decision (rainfall: the argmax class map; lightning: p ≥ LOW) with the post-processed one on the scope samples in three plots: per lead, the gain or loss of hits, misses and false alarms in % of the raw count, up meaning better; per lead, the difference post minus raw of the rates relative to the ground truth, hits, misses and false alarms each in % of the GT-active pixels, in percentage points; and, for the leads after the first, the change of each of those rates against the first lead, raw and post-processed side by side.
...
| `<track>_<scope>_<tag>_hysteresis.png` | Three plots. Per lead, the gain or loss of hits, misses and false alarms from the raw decision to the post-processed one, in % of the raw count, up meaning better, counts under the lead. Per lead, the difference post minus raw of the rates relative to the ground truth (each in % of the GT-active pixels), in percentage points. For the leads after the first, the change of each rate against the first lead, raw (hatched) and post-processed (solid). Raw is the argmax class map on the rainfall track and p ≥ LOW on the lightning track, recorded as `metrics_per_lead_raw`. Both tracks. |
