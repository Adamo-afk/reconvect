...
│   ├── opera_data/
│   │   ├── reflectivity/{YYYY}/{MM}/{DD}/*.h5      # OPERA max-reflectivity HDF5 (1 km, 5 min)
...
│   │   └── regrid_opera.py            # Reproject .h5 → Romania EPSG:31700 .nc
│   ├── raw_data/
...
│   ├── regridded_data/                # Cached reprojected data (generated by regrid.py / regrid_opera.py)
...
│   │   └── opera_data/{reflectivity|rainfall_rate}/nc4_{date}-Romania_{product}/*.nc
...
|---|---|---|---|
| Maximum reflectivity (dBZ) | 1 km | 5 min | HDF5 (`.h5`) |
| Instantaneous rainfall rate (mm/h) | 2 km | 15 min | HDF5 (`.h5`) |

Both products are listed in `product_cadences.json` as `opera_reflectivity: 5` and `opera_rainfall_rate: 15`. The rainfall-rate cadence (15 min) raises the validator floor to 15 min when OPERA is in use — comment them out (rename with a leading underscore) in the cadences file if you're not using OPERA and want a finer training step.
...
##### Step 3 — Reproject to the Romania grid (`regrid_opera.py`)

Reads each `.h5` file's `/where` metadata, builds a `pyproj.Proj(projdef)` source projection, projects via `pyresample` KD-tree onto the EPSG:31700 Stereo70 grid (1536×768), and saves a **CF-compliant NetCDF** under `our_data/regridded_data/opera_data/{reflectivity,rainfall_rate}/nc4_{date}-Romania_{product}/`. The KD-tree mapping is built **once per product** from the first file and reused across the rest.

```bash
# Both products
python our_data/opera_data/regrid_opera.py

# Only one product, single date, custom worker count
python our_data/opera_data/regrid_opera.py \
    --products opera_reflectivity --date 2025-06-15 --workers 8
```

Output schema per file (one `.nc` per source `.h5`):

| Variable | Dim | Notes |
|---|---|---|
| `max_reflectivity` or `rainfall_rate` | `(y, x)` | physical units; `nodata` masked to NaN; `undetect` set to 0 |
| `latitude`, `longitude` | `(y, x)` | EPSG:31700 grid coordinates as degrees |
| `crs` | scalar | `grid_mapping_name=oblique_stereographic`, `EPSG:31700`, proj4 string |

The output is directly viewable in Panoply / QGIS and downstream consumers can read it with `xarray.open_dataset(..., decode_coords='all')` to pick up the projection metadata.
