...
| File | Contents |
|---|---|
| `<track>_<scope>_<tag>_samples.csv` | One row per sample; both metrics × each lead time, `csi_t+<k>` per lead on the post-processed map at the tuned HIGH, `hits_pct` / `misses_pct` / `false_alarms_pct` per lead on the post-processed map at the tuned HIGH (hits and misses over the GT-active pixels, false alarms over the predicted-active pixels; empty where the denominator is zero), plus `hit_pct_t+<k>_ge<T>` — the percentage of GT pixels at or above threshold T that the prediction also puts at or above T, for every T of the sweep (base, base + step, … base × factor). |
| `<track>_<scope>_<tag>_summary.json` | `split`, `model`, `hit_thresholds_mmh`, `hit_pct_pooled_per_lead`, `representative_timesteps` (the best, worst and median sample by mean CSI over leads, with date, time, score and per-lead CSI; read by `--pick` in `predict_full_domain.py` and `visualize_gt_vs_pred.py`), totals, counts above `--high_coverage_pct` per lead × metric, FAR/POD/CSI per lead, initial selection list, tuned `post_processing` thresholds. |
...
| `hit_levels_<scope>/hit_levels_ge<T>.png` | For threshold T of the sweep: rows are the four seasons plus the whole window, columns the lead times, bars the share of samples whose hit percentage is at least 50 / 60 / 70 / 80 / 90 % (`--hit_levels`), one bar group per model. `<scope>` is `test` (`--split test`), `<yyyy>_<mm>` (`--year --month`), `test_<yyyy>_<mm>` (both) or `all_months` (neither: every whole-month validation run pooled). Seasons come from each sample's own date. |
