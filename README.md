...
| | |
|---|---|
| LOW | `--rainfall_low_threshold`, default `0.20` (`DEFAULT_RAIN_LOW`) |
| HIGH grid | `low + 0.01 … low + margin`, step `0.01` |
| Margin | `--rainfall_high_margin`, default `0.30` → 30 candidates, `0.21 … 0.50` |
...
The default margin spans the operational `DEFAULT_RAIN_HIGH` of 0.25, so the currently shipped setting is always inside the swept range and the sweep can only improve on it. Inference and the visualiser read the tuned rainfall pair (fixed LOW, HIGH per lead) from `--validation_summary`, as they already did for lightning. The per-lead winner maximises aggregate CSI; on a tie the **lower** threshold wins, which is the conservative choice. The sweep costs one connected-component labelling per sample and lead, not one per candidate: the components of the LOW mask do not depend on HIGH, and a component is kept at HIGH exactly when its maximum confidence reaches it, so every candidate is a boolean over components with identical results. Inputs and ground truth of the next sample are assembled in a loader thread while the GPU works on the current one, and the model is called with `predict_on_batch` (Keras `predict` traces a graph on the first call and wraps every call in a data adapter). Results land in `post_processing` in the rainfall summary, mirroring the lightning schema.
...
| `<track>_<scope>_<tag>_tuning.png` | CSI against the swept HIGH threshold, one line per lead, a dashed line at each lead's winner. Both tracks; absent for the baseline, which has no sweep. |
