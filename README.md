...
| **12** | `our_data/lightning_data/visualize_lightning_stats.py` | `--csv PATH` lightning activity index CSV to plot · `--summary PATH` coverage summary CSV for the monthly chart · `--output_dir PATH` destination for the generated bar charts | Per-day, per-timestep and per-month (active vs present-but-quiet, against the cadence expectation) lightning activity bar charts from step 2c's CSVs. Plots only — reads no model. | `python our_data/lightning_data/visualize_lightning_stats.py` |
...
| `our_data/lightning_data/summarize_lightning_data.py` | `our_data/lightning_data/`: `lightning_summary.csv` · `lightning_missing_timesteps.json` · `lightning_active_steps.csv` · `lightning_coverage.png` (`--chart`) | `intersect` (missing JSON) · `lightning_fraction` and `visualize_lightning_stats` (activity index) · chart is **terminal** | The intersection gates lightning on **presence**, like MTG and OPERA: a frame of zeros is an observation. A quiet day gets its zero frames from `read_kml_version2`, so it is a day, not an outage. `lightning_active_steps.csv` remains the activity index; `lightning_activity_monthly.png` shows the active/quiet balance per month. |
