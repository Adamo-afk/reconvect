...
**What it buys.** `<stem>_hysteresis.png` compares, per lead, the pooled hits, misses and false alarms of the raw decision (rainfall: the argmax class map; lightning: p ≥ LOW) with those of the post-processed one on the scope samples, as the gain or loss of each (up is better) and as the rates of both.
...
| `<track>_<scope>_<tag>_hysteresis.png` | What the hysteresis buys on the scope samples, per lead: the gain (+) or loss (−) of hits, misses and false alarms from the raw decision to the post-processed one, in % of the raw count, up always meaning better (more hits, fewer misses, fewer false alarms), as touching bars around the zero line with the counts underneath, and the pooled rates of both (hatched raw, solid post) over the tuned (LOW, HIGH). Raw is the argmax class map on the rainfall track and p ≥ LOW on the lightning track; both are recorded in the summary as `metrics_per_lead_raw`. Both tracks. |
