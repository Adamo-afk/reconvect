...
| `compare_models.py` | `comparison/<track>/metrics_per_leadtime_<metric>.png` · `comparison_table.{csv,md,tex}` · `hit_levels_<scope>/hit_levels.png` · `hit_levels_<scope>/hit_levels.csv` · `comparison_summary.json` | — (deliverable) | All models of a track on one axis: per-lead metric curves, the paper table, and the share of samples above each hit level per lead / season / whole window. |
...
| `comparison_table.csv` · `.md` · `.tex` | Paper-style table, one row per model, per-lead and aggregate columns; best value per column in bold (FAR: lower is better). |
