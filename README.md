...
**Picked timesteps:** `--pick csi --validation_summary <summary.json>` runs the timesteps that validation ranked best, median and worst by mean CSI over leads, or the top N with `--top_n`, under the same file names.
...
- **3-row full-domain figure** (`ts<NN>_<date>_<HHMM>.png`; with `--pick csi` `best_`, `median_`, `worst_` or `csi_top<NN>_` instead of `ts<NN>_`) — Row 1 GT, Row 2 raw prediction, Row 3 post-processing overlap. Green/red patch numbering shows which patches DBSCAN selected vs padded.
...
### `validate_predictions.py` → `validation/<stem>/`

Each run writes into its own folder named after its stem, `validation/<track>_<scope>_<tag>/`, and every file inside carries the same stem as a prefix, so a folder copied elsewhere stays self-describing. The readers (`compare_models`, `generate_report`, `build_patch_ensemble`) look for `<stem>/<stem>_*`; files left flat in `validation/` by older runs are ignored.
