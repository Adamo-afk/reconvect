...
**What it buys.** `<stem>_hysteresis.png` works in rates relative to the ground truth, hits, misses and false alarms each in % of the GT-active pixels, for the raw decision (rainfall: the argmax class map; lightning: p ≥ LOW) and the post-processed one on the scope samples. Top: per lead, the difference post minus raw of each rate, in percentage points, both rates under the lead. Bottom: for the leads after the first, the change of each rate against the first lead, raw and post-processed side by side.
