...
The lightning track already tuned its HIGH threshold over a 0.91–0.99 grid and persisted the result. The rainfall track does the same, shaped to its own scale, and sweeps **both** thresholds: every LOW candidate gets its own HIGH grid, and the winner per lead is the (LOW, HIGH) pair with the best pooled CSI. The scale matters: a 5-class softmax gives a rainy pixel 0.24–0.46 confidence on its winning class (median 0.28 on the f34 model), so thresholds that look natural for a two-class sigmoid leave the map empty.

| | |
|---|---|
| LOW grid | `0.20, 0.25, 0.30, 0.35` by default; `--rainfall_low_min` `--rainfall_low_max` `--rainfall_low_step` set a range; `--rainfall_low_threshold` alone fixes one LOW |
| HIGH grid, per LOW | `low + 0.01 … low + margin`, step `0.01` (`--rainfall_high_margin`, default `0.30`); `--rainfall_high_min` `--rainfall_high_max` name the range outright (a start at or below a LOW is lifted to one step above it); `--rainfall_sweep_step` sets the spacing |
| Winner | per lead, the (LOW, HIGH) pair with the best pooled CSI; ties go to the lower LOW, then the lower HIGH |
| Figure | `<stem>_tuning.png`: CSI against HIGH, one panel per lead, one line per LOW, the winner starred. `python validate_predictions.py --plot_tuning <summary.json>` redraws it from the saved summary, for either track, without model or data |

The fallback pair inference uses without a summary is `DEFAULT_RAIN_LOW` / `DEFAULT_RAIN_HIGH` = 0.20 / 0.25, inside the swept range. Inference and the visualiser read the tuned pair per lead from `--validation_summary`. The sweep costs one connected-component labelling per sample and lead, not one per candidate: the components of the LOW mask do not depend on HIGH, and a component is kept at HIGH exactly when its maximum confidence reaches it, so every candidate is a boolean over components with identical results. Inputs and ground truth of the next sample are assembled in a loader thread while the GPU works on the current one, and the model is called with `predict_on_batch` (Keras `predict` traces a graph on the first call and wraps every call in a data adapter). Results land in `post_processing` in the rainfall summary, mirroring the lightning schema.
...
| `validate_predictions.py` | `validation/<track>_[<split>_][<yyyy>_<mm>_]thr<T>mmh_<tag>_summary.json` · `..._samples.csv` · `..._metrics.png` · `..._hmf.png` · `..._<date>_<HHMM>.png` | `generate_report` · `build_patch_ensemble` · `bundle_eval_scores` | Per-lead POD / FAR / CSI with the hysteresis HIGH tuned by maximising aggregate CSI, plus a `per_patch` block. That block is what the ensemble selector reads to decide **which member wins each patch**. `--baseline` writes the same files for SepConv-ens under the `sepconv_<run_tag>` tag. |
...
**Picked timesteps:** `--pick best worst median --validation_summary <summary.json>` runs the timesteps that validation ranked best, worst and median by mean CSI over leads, under the same file names.
...
- **3-row full-domain figure** (`ts<NN>_<date>_<HHMM>.png`, or `<pick>_<date>_<HHMM>.png` with `--pick`) — Row 1 GT, Row 2 raw prediction, Row 3 post-processing overlap. Green/red patch numbering shows which patches DBSCAN selected vs padded.
...
| `<track>_<scope>_<tag>_summary.json` | `split`, `model`, `representative_timesteps` (the best, worst and median sample by mean CSI over leads, with date, time, score and per-lead CSI; read by `--pick` in `predict_full_domain.py` and `visualize_gt_vs_pred.py`), totals, counts above `--high_coverage_pct` per lead × metric, FAR/POD/CSI per lead, initial selection list, tuned `post_processing` thresholds. |
| `<track>_<scope>_<tag>_metrics.png` | Left: grouped FAR/POD/CSI bars per lead. Right: per-sample coverage scatter (IoU vs class-weighted, marker per lead), with red lines at 50 % on both axes. |
| `<track>_<scope>_<tag>_tuning.png` | Pooled CSI against HIGH, one panel per lead, one line per LOW, the winning pair starred. Both tracks; re-drawable with `--plot_tuning`. |
| `<track>_<scope>_<tag>_hmf.png` | Three panels, hits % / misses % / false alarms %, each a per-sample scatter in chronological order with a marker per lead, the pooled value per lead as a dashed line in the lead's colour and a red line at 50 %. Rainfall track only. |
...
| `RAINFALL_HIGH_MARGIN` | `0.30` | `--rainfall_high_margin` (or `--rainfall_high_min` / `--rainfall_high_max` for an explicit range) | How far above each LOW its HIGH grid runs: `low+0.01 … low+margin`. |
| `RAINFALL_LOW_GRID_DEFAULT` | `0.20, 0.25, 0.30, 0.35` | `--rainfall_low_min` / `--rainfall_low_max` / `--rainfall_low_step`, or `--rainfall_low_threshold` for one | The LOW candidates of the rainfall sweep. | Wider → more candidates, longer tuning. Narrower → may exclude the optimum. |
