...
| `<track>_<scope>_<tag>_tuning.png` | CSI against the swept HIGH threshold, one line per lead, a dashed line at each lead's winner; the panel the lightning metrics figure carries, as its own figure for rainfall too. Re-drawable from the summary with `--plot_tuning`. |
| `<track>_<scope>_<tag>_hmf.png` | Three panels, hits % / misses % / false alarms %, each a per-sample scatter in chronological order with a marker per lead, the pooled value per lead as a dashed line in the lead's colour and a red line at 50 %; the legend states each lead's pooled rate (summed pixel counts) and the mean of the per-sample rates. Rainfall track only; re-drawable with `--plot_hmf`. |
