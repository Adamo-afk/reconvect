...
| `<track>_<scope>_<tag>_hysteresis.png` | Three plots, all in % of the GT-active pixels (hits = TP, misses = FN, false alarms = FP over the GT-active pixels). Per lead, the rates of the raw decision (hatched) and of the post-processed one (solid), counts under the lead. Per lead, the difference post minus raw of each rate, in percentage points. For the leads after the first, the change of each rate against the first lead, raw and post-processed. Raw is the argmax class map on the rainfall track and p ≥ LOW on the lightning track, recorded as `metrics_per_lead_raw`. Both tracks. |
