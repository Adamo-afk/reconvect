...
An encoder-forecaster (ResBlock + ConvGRU) ingests a multi-resolution stack of MTG FCI satellite channels, OPERA composite radar, and LINET lightning, and predicts the next four 15-minute steps: **t+15 … t+60 min**.
...
- **Sample window:** a property of the dataset (`sequence_meta`); the current window is 3 past steps (t−30, t−15, t0) → 4 future steps (t+15 … t+60).
...
|---|---|---|---|---|
| `past_hr` — higher resolution | 1 km | 256 × 256 | none | MTG `vis_06`; LINET `density`, `current`, `occurrence` |
| `past_mr` — minimum resolution | 2 km | 128 × 128 | 2 × 2 avg | OPERA `reflectivity`, `rainfall_rate`; MTG `ir_38`, `ir_105`, `wv_63`, `wv_73` |
...
‡ **Baseline comparison pair, radar-only by design** — no MTG, no LINET. Modality enrichment is what RECONVECT is being credited for, so neither the baseline nor its ablation receives it. Both take **`opera_rainfall_rate_hr` alone, in HR at 256 px**, so the two models see the same tensor and a gap between them is attributable to the architecture. The HR form is also structural: a model's output resolution is its **finest input**, and this is the only mode with no other HR channel to hold it at 256. `opera_sepconv_logz` needs a **past=4** window (see the composition scheme). The modality question is answered separately, against the full model, by the ablation ladder in [Table 2](#table-2--validation-inference-visualisation--analysis). See [SepConv-ens baseline](#sepconv-ens-baseline).
...
|---|---|
| Lightning | Hysteresis at the evaluation's LOW and the tuned per-lead HIGH |
| Rainfall | Hysteresis on `p(argmax)` at the tuned per-lead (LOW, HIGH) |

**What the validation scores, and why.** The extraction predicts the split's *records*: each reference's DBSCAN-selected patches are read from the TFRecords, already cut and normalised, predicted in batches exactly as the evaluation does, and pasted back onto the 768 × 1536 canvas at their slots. The hysteresis runs on that canvas, so a component crossing a patch edge is still one component, and the ground truth is the records' labels. Pixels outside the selected patches are outside the scoring: the validation, like the evaluation, measures the model on the storm patches the dataset was built from, not on the dry remainder of the domain, and false alarms in quiet areas do not enter it. This is deliberate. It keeps the validation on the same population as the evaluation, whose lightning threshold it inherits, and it costs one pass over the records, about 5,300 patches per split for f34, instead of every patch of every reference from the reprojected fields. Two things follow. The lightning probabilities under the hysteresis are each patch's own output, without the Hann blend across overlapping positions, which belongs to the inference and the per-date visualisation. And a validation summary is not comparable to one made before this change, which scored the whole canvas; the summary's `selection_criterion` and `sampling` blocks say which population a run used.
...
The fine-tune stage freezes the base model and trains a correction head on its decoder features `F` and its logits `L_b`: `logits_l = s_l · L_b[l] + head_l(x)`, with the four per-lead 1 × 1 heads at zero and `s_l = 1` at start, so step 0 is the frozen model and training can only move away from it where the loss improves. The head is a two-level Swin-UNet: `F` collapsed over time and embedded at 128², level 1 with 8 × 8 windows at 1 km, a 2 × 2 patch merge to 64², level 2 at 2 km where one window spans about 32 km, then a bilinear upsample with the level-1 skip and a pixel-shuffle expand back to 256². The Swin blocks carry the relative position bias and the shifted-window mask. Two raw stems bypass the backbone: the last two frames of the target field at 1 km (the LINET channels for lightning, OPERA rain rate and reflectivity for rain) join at the output, so persistence is one linear step away, and the last two MR frames join at level 1.

**Two variants.** `swin_unet` is deterministic and is the lightning head. `swin_unet_cvae` writes its own artefacts, `coalition_<run_tag>_finetuned_cvae.keras`, `history_<run_tag>_finetuned_cvae.json` and the checkpoint `<run_tag>_finetune_cvae_latest`, so both rain heads live in one models folder; the consumers select it with `--cvae` (evaluation, inference, visualiser) or `mode:cvae` (validation). It adds a conditional VAE for the rain track: a prior network `p(z | c)` on the level-2 features and a posterior network `q(z | c, y)` that also sees the future, both Gaussian over a `16² × 32` latent; training samples `z` from `q`, inference from `p`; `z` modulates the level-2 features through a gated FiLM, `c ← c + σ(g(c)) ⊙ (γ(z) ⊙ c + β(z))`, zero-initialised, the gate starting mostly closed. The loss adds `beta · mean_i max(KL_i, free bits)` with a linear warm-up of `beta`. The rain track is trained deterministically first, as the baseline, then with the cVAE, and the two are compared on the heavy classes at the long leads.

**Losses.** Lightning: BCE with a positive-class weight `min(N_neg/N_pos, 30)`, focal `γ = 2` as the fallback. Rain, deterministic: the weighted focal CE of `[radar_loss]` plus `0.2 ·` the expected-class L1, which pushes mass toward adjacent bins. Rain, cVAE: class-weighted CE without the focal factor, the reconstruction term of the ELBO.

**Monitoring** (in the history JSON, per epoch, train and validation): the loss and its terms; `csi_t<k>` per lead, `csi_mid` (the early-stopping monitor), `fss3_t<k>` and `fss9_t<k>`; `brier` and `persistence_csi_t<k>` for lightning, `adjacent_frac` for rain; `corr_ratio_t<k>` = ‖correction‖ / ‖L_b‖ and `lb_scale_t<k>`; `grad_norm_stage2`; for the cVAE `l_kl`, `active_units` (latent dimensions with KL above 0.02 nats), `gate_mean`, `gate_std`, `l_forecast_q` (validation with `z` from the posterior, the ceiling; the gap to `l_forecast` is the prior–posterior gap) and, on a few validation batches, the ensemble metrics `val_ens_*`: RPS, ensemble-mean CSI of the event and of the top two classes, member disagreement, spread–skill ratio and a rank histogram. `--max_batches N` with `--epochs E` runs E epochs on N batches per split and saves, to check a configuration end to end.

**Members.** A cVAE model produces one forecast per draw of the latent; `set_member(k)` draws member `k` from a fixed seed, `-1` is the prior mean. The validation (`--members`, default 3) tunes the thresholds on the prior mean, then scores every member per patch on the validation split at those thresholds and lets the best member win each patch; the scope is scored on that composite, the summary records the winner per patch under `members`, and `<stem>_members.png` shows one panel per member with its per-patch CSI, the patches it won in green. The visualiser and the inference read the same block through `--validation_summary` and compose their prediction the same way, patch by patch.

### Hysteresis tuning in validation

Both tracks tune their post-processing on samples drawn from the **validation split** and report every number on the scope (the test split, or a month). Nothing about the scored samples enters the choice.

The samples of every phase are the split's records, predicted as the evaluation predicts them and pasted back onto the canvas (see *What the validation scores* above).

**Rainfall, three phases.**

| Phase | What | Figure |
|---|---|---|
| 1 | LOW per lead: a plain threshold on p(argmax) of the rainy-argmax pixels, swept 0.01 … 0.99 through probability histograms; the best pooled CSI wins. `--rainfall_low_threshold` skips this and fixes one LOW for every lead. | `<stem>_tuning_low.png` — CSI against the threshold, one line per lead, a dashed line in the lead's colour at the chosen LOW, the value and its CSI in the legend |
| 2 | (LOW, HIGH) per lead: windows of `--rainfall_window` (default 0.02), tiled on both sides of the phase-1 LOW out to `--rainfall_reach` (default 0.10). For 0.35: (0.33, 0.35), (0.31, 0.33), … and (0.35, 0.37), (0.37, 0.39), …; every window is one hysteresis run; the best pooled CSI wins. | `<stem>_tuning_high.png` — one panel per lead, a bar per window, the best one full-colour with its CSI written on it |
| 3 | The scope is scored at the chosen pair per lead: FAR / POD / CSI, coverage, hits / misses / false alarms, per-patch CSI. | the metrics, coverage and hits figures |

The summary records `low_threshold_per_lead` and `high_threshold_per_lead`; inference and the visualiser apply them per lead through `--validation_summary`. The fallback without a summary is `DEFAULT_RAIN_LOW` / `DEFAULT_RAIN_HIGH` = 0.20 / 0.25.

**Which leads.** `--tune_leads each` (default) runs the tuning for every lead separately; `--tune_leads first` runs it on t+1 alone and reuses those thresholds for every lead, on both tracks, and the figures then show the one tuned lead.

**Lightning.** LOW is the decision threshold the evaluation tuned for the model, read from `evaluation/eval_<tag>/evaluation_results.json` (`--lightning_low_threshold` overrides it), or, with `--lightning_tune_low`, a phase 1 sweeps it per lead on the validation split exactly as the rainfall track does, 0.01 to 0.99 as a plain threshold on p, the best pooled CSI winning, drawn in `<stem>_tuning_low.png`; HIGH is then swept above each lead's LOW in 0.01 steps per lead on the validation-split samples (`<stem>_tuning.png`), and the scope is scored at the chosen HIGH per lead. The evaluation itself draws `threshold_sweep.png`: pooled CSI against the decision threshold on the validation split, where it was chosen, and on the test split, with the operating point and the test optimum marked.

**What it buys.** `<stem>_hysteresis.png` compares the raw decision (rainfall: the argmax class map; lightning: p ≥ LOW) with the post-processed one on the scope samples in three plots of the same rates as the other hits / misses / false-alarm figures, hits and misses in % of the GT-active pixels, false alarms in % of the predicted-active pixels: per lead, the rates themselves, raw and post-processed side by side; per lead, the difference post minus raw of each rate in percentage points; and, for the leads after the first, the change of each of those rates against the first lead, raw and post-processed side by side.
...
**Cost.** The extraction never reads the reprojected fields: it predicts the split's records with a `tf.data` pipeline (`--datasets_root`, `--batch_size`), one pass per phase over the records of the split, filtered to the selected references. The maximum rain rate of every OPERA field the sample selection has looked at is kept in `our_data/data_statistics/opera_rainfall_max_cache.json`, trusted while the file's size and modification time match, so the next run selects its samples without reopening the files. On the rainfall track the tuning samples go through the model once: phase 2 replays the phase-1 predictions from memory, up to `--cache_gb` (default a third of the physical memory), and only past that budget predicts them again. None of this changes a number.
...
compress_datasets --move TAG --to ROOT
  └─ copies the archive to ROOT, verifies it, extracts it there and
     re-registers the dataset under the new root (source is removed)
```

| Command | Effect |
|---|---|
| `--move TAG… --to ROOT` | Carry one or more compressed datasets to another datasets root and decompress them there. The registry entry follows, so `train_models --datasets_root ROOT` finds them without a restore. |
...
**Deletion.** The source is removed only after 7-Zip exits 0, `7z t` passes, and the archive's file count matches the disk. Between the archive finishing and the reclaim, the dataset and its archive coexist on disk.
...
Every script that reads a tagged artefact takes the flag.

Two scripts stay period-less by design: `verification_keys.py` spans *several*
windows and so takes `--reconvect_tag` (one or more) / `--sepconv_tag`, and
...
`datasets/` resolves **separately** from `data_root`: the patch pool and
reprojected archive are terabytes that stay put, while a TFRecord dataset is
tens of gigabytes that may sit on whichever disk has room.
...
Pass the same roots to every stage of a run; exporting the environment
variables once does that.
...
**Deletion.** Each file is compressed to a `.tmp`, read back off disk and compared byte for byte against the original before the original is unlinked. What is stored is the entire `.npy` file, header included, so a restore is byte-identical. Re-running skips files whose target already exists, so an interrupted pass resumes.
...
Routed through it: `reproject.py`, `extract_patches.py`, `identify_patches.py`, `compute_normalization_stats.py`, `create_datasets.py`, `lightning_fraction.py`, `opera_rainfall_fraction.py`, `validate_predictions.py`, `store_registry.py`, and the three summarizers. A compressed frame cannot be memory-mapped.

### Feature-extractor leakage gate

When `--stage finetune` freezes a base model as a feature extractor, the period recorded in its `.meta.json` sidecar is compared against the dataset's `metadata.json`. Any shared date aborts the run unless `--allow_period_overlap` is passed; a model without a sidecar reports its period as unknown and warns.
...
The invariant is that **training and inversion use the same constants** — a z-value only means something relative to the `mean`/`std` that produced it. RECONVECT's training split contains 36 of the baseline's test timestamps, so scoping per split also keeps each model's test data out of the constants defining its space.
...
Both share `_ResumableCheckpoint` and the `[checkpointing]` section, resume
at the right epoch with optimizer state intact, and take `--fresh` to ignore
a checkpoint. Each base model keeps its own checkpoint file.
...
when early stopping cut it. The baseline resolves all three base models
internally, and `--weights latest` on `--finetuned` picks the finetune stage's
own checkpoint.
...
Both files are scoped to the same set of training timesteps: `compute_normalization_stats.load_training_keys` and `lightning_fraction.load_scope_set` return identical key sets for a given tag, both anchored on `reference_utc` so windows through midnight expand correctly.

Several windows coexist on disk, each with its own statistics, so `sepconv_predict.to_mmh` takes `stats_period` as a required argument and `verification_keys.py` requires `--sepconv_tag`. The baseline does not use `create_datasets.py --global_stats`.

### Sample selection is shared, splits are not

Both models draw from the same `patch_index.csv`, DBSCAN-gated at the threshold recorded in `patch_index.json` (8 mm/h for the current index) — that gate is upstream of both. Within a selected patch every pixel is used by both, including the 99.8 % dry ones; there is no per-pixel thresholding. **The weighted loss exists because of that**: plain MSE on this distribution is minimised by emitting the dry value everywhere, at a cost of 0.09 versus 91.6 under the weighting.

The splits differ only because the windows do — `past=4/future=4` needs 9 consecutive timesteps against `w34`'s 8, so fewer references qualify. The Czibula splitter then assigns by position within each 6-hour block, which puts the same key on opposite sides:
...
The intersection removes 12–13 % of each test split; 380 of the baseline's test keys were seen by RECONVECT during fitting.

The frozen file is enforced on **both** sides of the comparison — `evaluate_sepconv_ensemble.py --verification_keys PATH` and `evaluate_coalition.py --verification_keys PATH`. Pass the same file to both. It matches on the `date` and `reference_utc` fields carried in the TFRecord shards.
...
A patch file is an array of tiles in the order of the index's active-patch list, so a rebuilt index reorders the slots. Two guards keep the pool consistent with it:

| guard | where | what it does |
|---|---|---|
| **Stamp** | `our_data/patches/<date>/_patch_index.json` | Records the active-patch list every file in that date was built from, plus the index's SHA-256. `extract_patches` compares per timestep and **re-extracts** any whose list has moved. Merged on write, so a `--date` run doesn't erase the rest. |
| **Raise** | `create_datasets.StalePatchPool` | An out-of-range `idx_t*` raises, naming the date, timestep, variable, requested slot and actual length. A missing variable stays zero-filled, since a mode may reference a product with gaps. |

```bash
python extract_patches.py --audit_pool [--period TAG]    # report drifted dates, extract nothing
```

Overwriting a stale file also deletes its `.npy.zst` twin.
...
# several RECONVECT windows at once -> one set every model is scored on:
...
Each model is normalised by its own split, so `--global_stats` is absent from both builds. The two `extract_patches` runs write into the same shared `patches/` tree, so the second is largely a no-op over the overlap. The ablation takes `opera_rainfall_rate` only — one channel, matching the baseline.

Reclaim the ~86 GB of TFRecords afterwards with `python compress_datasets.py --reclaim-all`.

### Comparison scope

`compare_models.py --track rainfall --include_baseline` puts the baseline on the same axes and in the same table as every RECONVECT rainfall model (the baseline is first validated with `validate_predictions.py --baseline --period w44`, which post-processes its mm/h output into the same classes).
...
Requires **Conda**, an **NVIDIA GPU**, and **Windows**. Follow this order.

1. **Install CUDA 11.2 + cuDNN 8.1 via the NVIDIA installers** (not conda), then set `CUDA_PATH`, which is how TensorFlow finds CUDA on Windows:
   ```powershell
...
2. **Create the env** (3.10 is the last Python with native Windows TF GPU support):
   ...
   ```
3. **Geospatial stack via conda-forge first**, so `pyproj` / `proj` / `geopandas` / `cartopy` / `pyresample` share one PROJ ABI:
   ```powershell
...
> **Tables as images.** The long tables below are shown as rendered images so they read at desktop width; the markdown source of each sits in a collapsed *Text version* block directly under it. The text is what gets edited when something changes, and the PNG in `assets/` is re-rendered from it.
>
...
> | `YYYY-MM-DD` | `linet_export.py`, `read_kml_version2.py`, `reproject.py`, `identify_patches.py`, `extract_patches.py`, `predict_full_domain.py`, `validate_predictions.py`, `evaluate_*.py` | **inclusive** everywhere |
> | `HH:MM` · integer | `--time`, `--start-time`, `--end-time` · `--hour` 0–23, `--year`, `--month` 1–12 | — |
>

<p align="center"><img src="assets/table1_training.png" width="100%" alt="Table 1 — Training a model from scratch"/></p>

<details>
<summary>Text version (source of the image — edit this, then re-render)</summary>
...
| **1b** | `our_data/opera_data/pipeline_opera.py` | `--start` `--end` `yyyy/mm/dd-hhmm` window, **end inclusive** · `--ssh_key PATH` SSH private key; excludes `--password_file` · `--password_file PATH` text file holding the SSH password · `--products` reflectivity, rainfall_rate, or both by default · `--remote_base PATH` remote EWC mount root directory · `--remote_host` `--remote_user` SSH endpoint and login account · `--cache_dir PATH` local OPERA destination root directory · `--timesteps` override the per-product minute filter | Fetches OPERA composite HDF5. `--remote_base /home/eumetsatdata` on EWC images whose mount root differs from the default `/eumetsatdata`. | `python our_data/opera_data/pipeline_opera.py --start 2025/05/01-0000 --end 2025/05/31-2359 --ssh_key `$\color{red}{\textbf{\textit{path/to/ssh-key}}}$<br>`… --remote_base /home/eumetsatdata` |
| **1c** | `our_data/lightning_data/linet_export.py` | `--start` `--end` `YYYY-MM-DD` period, both inclusive · `--format` txt point list, kml, or asc · `--out PATH` destination root for the exported strokes · `--bbox` lon/lat rectangle limiting the export area · `--password_file PATH` text file holding the LINET password · `--lightning-type` 0 all, 1 cloud-to-ground, 2 intracloud · `--amp-threshold` minimum stroke amplitude to keep · `--daily-window` split the request into per-day windows · `--pause` seconds to wait between successive requests · `--force` re-download even when the output exists · `--dry-run` plan the fetch without downloading anything | Downloads LINET strokes. Use `--format kml`: it writes `{out}/kml_data/YYYY-MM-DD/…` which the rasteriser reads directly. | `python our_data/lightning_data/linet_export.py --start 2025-05-01 --end 2025-05-31 --format kml --out our_data/lightning_data --password_file `$\color{red}{\textbf{\textit{creds.txt}}}$ |
...
| **2c** | `our_data/lightning_data/summarize_lightning_data.py` | `--start` `--end` `YYYY-MM-DD` range the cache should cover · `--data_dir PATH` rasterised lightning `.npy` root to scan · `--workers N` parallel workers for the per-date scan · `--output PATH` per-date coverage summary CSV destination · `--missing PATH` missing-timesteps JSON destination · `--active PATH` per-timestep activity index CSV destination · `--chart [PATH]` monthly coverage chart PNG | Same for LINET → `lightning_summary.csv` + `lightning_missing_timesteps.json` + `lightning_active_steps.csv`. Every array is read in full to test for a non-zero pixel, so the scan is disk-bound and parallel by default. The intersection gates on the missing JSON (presence); the activity index feeds `lightning_fraction.py --scope_csv` and `visualize_lightning_stats.py`. | `python our_data/lightning_data/summarize_lightning_data.py --start 2025-01-01 --end 2026-08-13 --chart`<br>`python our_data/lightning_data/summarize_lightning_data.py -w 12` |
| **3** | `intersect_product_coverage.py` | `--summary name=PATH` per-product coverage summary CSV; `name` is `mtg`, `lightning`, `opera_rainfall_rate` or `opera_reflectivity` (`opera` is an alias for the former) · `--missing name=PATH` per-product missing-timestep JSON · `--errors_log PATH` reprojection error log to subtract · `--timestep_config PATH` master cadence config to validate against · `--output_csv PATH` where the timestep manifest is written · `--output_plot PATH` destination for the coverage bar chart | Intersects per-product coverage into `timestep_manifest.csv` — the timesteps where *all* requested products exist. Gates step 5. **The requested set is your choice**, and OPERA's two fields are named separately so a manifest requires only what its modes read: both OPERA keys draw on the same summary CSV and missing JSON, selecting different blocks. | `python intersect_product_coverage.py --summary mtg=our_data/satellite_data/mtg_summary.csv --summary opera_rainfall_rate=our_data/opera_data/opera_summary.csv --summary opera_reflectivity=our_data/opera_data/opera_summary.csv`<br>`python intersect_product_coverage.py --summary opera_rainfall_rate=our_data/opera_data/opera_summary.csv --output_csv our_data/timestep_manifest_rain.csv` |
| **4** | `identify_patches.py` | `--threshold` mm/h rain-rate floor for DBSCAN clustering; default = the master index's recorded value, else 10 · `--sweep [MMH…]` visualisation only: the share of active samples and the patches per active sample against the threshold, every `--sweep_stride`-th timestep (default 4), the threshold in force marked; writes `plots/threshold_sweep.png` and `.csv` · `--rule pixel\|box` how clusters become tiles (see the thresholds reference); default = the master index's recorded rule, else `pixel` · `--eps` DBSCAN neighbourhood radius, in pixels · `--min_samples` minimum pixels needed to accept a cluster · `--data_root PATH` root holding the reprojected OPERA data · `--output_dir PATH` destination for the patch index CSV/JSON · `--date YYYY-MM-DD` single date; excludes `--start`/`--end` · `--start` `--end` `YYYY-MM-DD` range, **both bounds inclusive** · `--plot` render the day into two GIFs, one frame per timestep (requires `--date`) | DBSCAN over OPERA `rainfall_rate`; marks which of the 18 patches are convectively active per timestep → `patch_index.csv`. | `python identify_patches.py`<br>`python identify_patches.py --start 2025-05-01 --end 2025-05-31`<br>`python identify_patches.py --date 2025-05-14 --plot` |
...
| **10** | `train_models.py` | `--config PATH` training config carrying all hyperparameters · `--mode` train one mode instead of the list · `--stage base\|finetune\|both` base training, the fine-tune head, or both · `--head swin_unet\|swin_unet_cvae\|swin_legacy` the fine-tune head, overriding `[finetune].head` · `--batch_size N` override · `--max_batches N` dry run on N batches per split, then save · `--epochs N` override the configured epochs · `--base_checkpoint PATH` frozen backbone for the finetune stage · `--dataset_dir PATH` override the derived dataset directory · `--output_dir PATH` destination for saved models and history · `--data_root PATH` root holding datasets and loss priors · `--fresh` ignore any saved per-epoch checkpoint · `--list-modes` print the mode registry and exit | Builds the encoder-forecaster from `metadata.json` and trains. `finetune` freezes the backbone and grafts a Swin head; `both` runs the two back-to-back in one process. Resumes from the per-epoch checkpoint unless `--fresh`. | `python train_models.py --list-modes`<br>`python train_models.py --mode mtg_opera_mtgmr_rainfall --stage base`<br>`python train_models.py --mode mtg_lightning_opera_occurrence --stage both`<br>`python train_models.py --config training.config`<br>`python train_models.py --mode mtg_opera_mtgmr_rainfall --stage base --fresh` |
| **11** | `train_lightning_kd.py` | `--period` the teacher's period label (its checkpoint and dataset; the student is tagged the same) · `--teacher_mode` `--student_mode` the pair (defaults `mtg_lightning_opera_occurrence` → `mtg_opera_occurrence`) · `--datasets_root` where the teacher's dataset lives · `--fresh` ignore the student's per-epoch checkpoint (`checkpoints/<student_run_tag>_kd_latest.keras`, written every epoch and resumed from by default) · `--kd_alpha` weight on the soft-teacher distillation loss · `--kd_temperature` softening temperature for both models' outputs (default 2) · `--kd_soft_weight teacher\|none` weight each pixel's soft loss by the raw teacher probability plus `--kd_weight_floor` (default 0.01), or not · `--eval_root` where the training curves are drawn after the run · `--config` the `training.config` whose `[lr_schedule]` gives the student the same cosine-with-warmup schedule as the other models (`--learning_rate` overrides its `initial_lr`) · `--teacher_finetuned` distil from the Swin teacher instead · `--epochs` `--batch_size` training length and samples per step · `--learning_rate` Adam learning rate for the student · `--patience` early-stopping patience, counted in epochs · `--shuffle_buffer` samples held in RAM for shuffling · `--seed` RNG seed for reproducible student training · `--no_mixed_precision` disable fp16 compute on tensor cores · `--data_root` `--model_dir` dataset root and checkpoint destination | **Optional.** Distils the teacher into `mtg_opera_occurrence`, a student predicting lightning from satellite + OPERA with **no LINET at inference**. The soft loss is the per-pixel KL between the temperature-softened teacher and student (the teacher's own entropy, a constant, is not part of the logged value), weighted toward the pixels where the teacher has an opinion, times T²; the hard loss is the teacher's own focal loss against LINET; total = α·soft + (1−α)·hard. After the run the training curves are drawn to `evaluation/eval_<student_tag>_kd/` with the exact values at the best and the last epoch in the legend. | `python train_lightning_kd.py`<br>`python train_lightning_kd.py --kd_alpha 0.5 --kd_temperature 6.0`<br>`python train_lightning_kd.py --teacher_finetuned` |
| **12** | `sepconv_ensemble_training.py` | `--period` window tag of the baseline dataset · `--lead 1\|3\|5` train one base model, not all three · `--epochs` `--batch_size` training length and samples per step · `--learning_rate` published value 1e-3; exposed for the sweep · `--lr_patience` epochs on a plateau before halving · `--es_patience` early-stopping patience; ours, not the paper's · `--data_root` `--model_dir` dataset root and checkpoint destination | **SepConv-ens baseline.** Three radar-only regression models (Bm1/Bm3/Bm5, named by their lead in steps past their own window) composed to **t+1…t+4 = 15–60 min** from observations alone — nothing autoregressive. Consumes `opera_sepconv_logz`, which needs a past=4 window because t+2 and t+4 read `t−4`. See [SepConv-ens baseline](#sepconv-ens-baseline). | `python sepconv_ensemble_training.py --period w44`<br>`python sepconv_ensemble_training.py --period w44 --lead 1` |
| **13** | `verification_keys.py` | `--reconvect_tag TAG…` window tag(s) of the RECONVECT split(s); several tags give one set every model is scored on · `--sepconv_tag` window tag of the baseline split · `--write` freeze the key set to JSON · `--output PATH` frozen key-set destination · `--data_root PATH` root holding the split CSVs | Builds the leakage-free key set shared by RECONVECT and the baseline: intersect every named test split, then subtract every key any of their train or validation splits holds. The file name lists the windows, e.g. `verification_keys_dbscan_f34_w34_vs_w44.json`. **Run with `--write` before the test data is scored.** | `python verification_keys.py`<br>`python verification_keys.py --write` |
| **14** | `compress_datasets.py` | `--compress TAG` archive, verify, delete shards · `--restore TAG` extract back · `--reclaim TAG` drop the on-disk copy of an archived dataset · `--reclaim-all` sweep leftovers · `--move TAG… --to ROOT` carry a compressed dataset to another datasets root and decompress it there · `--jobs` background job state · `--background` detach the job · `--workers N` 7-Zip threads · `--level` `-mx` compression level · `--keep` archive without deleting | Dataset archiving with 7-Zip — shards compress to ~4.8 % at `-mx=5`. Deletion only after the archive verifies. See [Dataset archiving](#dataset-archiving). | `python compress_datasets.py`<br>`python compress_datasets.py --compress <run_tag> --background`<br>`python compress_datasets.py --reclaim-all`<br>`python compress_datasets.py --move <run_tag> --to E:\nowcasting\datasets` |
...
| Extraction | Fetched chunks are extracted to `.npy` in the same run. |
...
Cycles that fail to extract keep their raw for the `--reprocess` retry.
...
Extraction skips cycles whose `.npy` already exist, and `--delete_raw`
still reclaims them; `--reprocess` forces re-extraction.

`--provenance nma|datastore` stamps what the run extracts; raw in
`_raw_chunks/` carries no evidence of its origin, so without the flag
nothing is recorded. Entries already in the ledger are never overwritten.
...
them up front, which is right for a model that reads reflectivity.
...
### Declare the expected range

All three summarisers take `--start` / `--end`, the range the archive is
expected to cover; a date in range with no files at all is then reported
as missing, and the Data Store backfill can request it.
...
Cycles from before the ledger existed report as `unrecorded`; the two
sources share filenames, so their origin is not guessed at.
...
Nothing is verified in that mode; the summary, run beforehand, holds that
judgement. Deletion is confined to the requested window.

Deletion is irreversible: afterwards `--scan npy` is the only valid
coverage view, and `--source local` can no longer re-derive `.npy` from
raw. Compare `--scan raw` and `--scan npy` over the same range before
deleting, and run `--source local` first if raw exceeds `.npy`.

### OPERA SFTP notes (step 1b)

The EWC VM takes key authentication (`--ssh_key`), and `--remote_base`
falls back to the EWC mount root on its own.
...
<p align="center"><img src="assets/table2_validation_inference.png" width="100%" alt="Table 2 — Validation, inference, visualisation & analysis"/></p>
...
| # | Script | Arguments | What the step does | Commands |
|---|---|---|---|---|
| **1** | `evaluate_coalition.py` | `--mode` model variant to evaluate; required · `--split train\|validation\|test` which dataset split to score · `--cvae` evaluate the conditional-VAE fine-tuned head · `--finetuned` \| `--kd` evaluate Swin head or KD student · `--threshold` fixed decision threshold; else optimised on validation · `--plot_threshold` probability floor for the lightning visualisation · `--date YYYY-MM-DD` `--hour 0-23` reference for the sample figure · `--batch_size` samples per inference step · `--data_root` `--model_dir` `--output_dir` dataset, checkpoint and results locations | Full metric suite on a held-out split → `evaluation/eval_<run_tag>/evaluation_results.json` + plots. Leads follow the dataset's label depth (a 3 / 4 window scores four). Training curves come as one figure per quantity (`training_loss.png`, `training_<metric>.png`) with the best-epoch and last-epoch cut-off lines read from the history and the latest checkpoint sidecar, and the exact value at those two epochs in the legend. Per-lead metrics feed the coalition study and `compare_models.py`. | `python evaluate_coalition.py --mode mtg_opera_mtgmr_rainfall`<br>`python evaluate_coalition.py --mode mtg_lightning_opera_occurrence --finetuned`<br>`python evaluate_coalition.py --mode mtg_opera_occurrence --kd`<br>`python evaluate_coalition.py --mode mtg_opera_mtgmr_rainfall --split validation` |
...
| **3** | `validate_predictions.py` | `--track rainfall\|lightning\|kd` which validation pipeline runs; required · `--split train\|validation\|test` score the reference timesteps of that dataset split, read from its CSV · `--year` `--month 1-12` scan that calendar month, or restrict the split to it; required without `--split` · `--date YYYY-MM-DD` switches from extraction to visualisation mode · `--mode M…` model(s) whose predictions are validated; several run one after the other on the same track, paired with `--period P…` (one label serves all); an entry may name its variant, `mode:finetuned` or `mode:kd`, so the teacher and its distilled student run together · `--finetuned` \| `--kd` validate Swin head or KD student · `--datasets_root` where the split records live (default `<data_root>/datasets`) · `--stride` Hann overlap stride of the per-date lightning visualisation · `--lightning_tune_low` lightning: sweep LOW per lead on the validation split (phase 1) instead of taking the evaluation's threshold · `--lightning_low_threshold` override of the hysteresis LOW; by default the evaluation's tuned threshold, read from `--eval_root/eval_<tag>/` · `--rainfall_threshold_mmh` rain-rate cut for sample selection · `--high_coverage_pct` coverage grade recorded in the summary · `--baseline` validate the SepConv-ens baseline instead of a RECONVECT model (post-processed to classes, no hysteresis) · `--baseline_period TAG` add the baseline to a multi-model run, after the `--mode` models · `--max_samples N` cap the scored and the tuning samples at N, drawn reproducibly per month with `--month_batch` from each month in turn and `--seed` · `--rainfall_window` `--rainfall_reach` the phase-2 windows of the rainfall tuning · `--tune_leads each\|first` tune every lead or t+1 alone and reuse it · `--workers` threads for the per-lead sweeps · `--weights best\|latest` the final save or the per-epoch checkpoint; the artifact tag of a `latest` run ends in `_latest`, so both saves can be validated side by side · `--plots SUMMARY.json` redraw every figure of a finished run from its summary and samples CSV, either track, and exit ·  `--teacher_mode` `--student_mode` mode pair for the KD track · `--teacher_finetuned` `--no_student_kd` variant toggles for the KD pair · `--batch_size` samples per inference step · `--data_root` `--model_dir` `--output_dir` dataset, checkpoint and results locations | **Extraction (no `--date`):** every output carries the scope and the validated model's artifact tag (`<track>_[<split>_][<yyyy>_<mm>_]thr<T>mmh_<tag>_*`, T the selection threshold), so the test split and single months of several models, at any threshold, coexist and can be compared afterwards. Takes the reference timesteps of the split or of the month, keeps those with ≥1 pixel at or above the selected threshold (`--rainfall_threshold_mmh`), runs inference, tunes the per-lead hysteresis HIGH by maximising aggregate CSI, writes CSV + summary JSON + metrics figure. **Visualisation (`--date`):** plots overlays for that day using the tuned thresholds. `kd` runs teacher and student on identical samples and tunes each independently. | `python validate_predictions.py --track rainfall --split test --mode <mode> --period f34`<br>`python validate_predictions.py --track rainfall --year 2025 --month 5`<br>`python validate_predictions.py --track rainfall --year 2025 --month 5 --finetuned`<br>`python validate_predictions.py --track lightning --year 2025 --month 5 --mode mtg_lightning_opera_occurrence`<br>`python validate_predictions.py --track lightning --year 2025 --month 5 --mode mtg_lightning_opera_occurrence --date 2025-05-14`<br>`python validate_predictions.py --track kd --year 2025 --month 5` · `--mode M:cvae` the cVAE fine-tuned head of a mode (next to `M:finetuned` and `M:kd`) · `--members N` cVAE rain head: draws of the latent scored per patch on the validation split, the best member wins each patch (default 3, 0 disables) · `--cache_gb` memory budget for replaying the phase-1 predictions in the rainfall phase 2 (default a third of the physical memory, 0 disables) |
| **4** | `predict_full_domain.py` | `--mode` model variant to run; required · `--weights best\|latest` the final save or the per-epoch checkpoint; a `latest` run's outputs carry a `_latest` suffix · `--date YYYY-MM-DD` reference date to predict; required unless `--pick` · `--validation_summary PATH` apply the tuned thresholds per lead: HIGH for lightning, LOW and HIGH for rainfall · `--baseline` run the SepConv-ens baseline of `--period` instead of a model, its class map through the same figures, no post-processing · `--pick csi\|active` run the `--top_n` samples (default 5) of the validation run in `--validation_summary`: the best by mean CSI over leads, or those with the most ground-truth-active pixels over the leads; outputs are prefixed `csi_top<NN>_` or `active_top<NN>_` so the two sets coexist (no ground truth needed; `--date` and the time flags are ignored) · `--time HH:MM` a single reference time · `--hour 0-23` every step-aligned reference within one hour · `--start-time` `--end-time` `HH:MM` inclusive reference-time range · `--cvae` the conditional-VAE fine-tuned head · `--finetuned` \| `--kd` run Swin head or KD student · `--validation_summary PATH` load tuned per-lead hysteresis thresholds · `--lightning_low_threshold` `--lightning_high_threshold` hysteresis pair, lightning head · `--rainfall_low_threshold` `--rainfall_high_threshold` hysteresis pair, rainfall head · `--stride` Hann overlap stride for lightning inference · `--patches` restrict to a patch subset, rainfall only · `--threshold` fixed binarisation threshold overriding the default · `--batch_size` samples per inference step · `--no-plot` `--save-npy` skip PNGs; dump raw prediction canvases · `--data_root` `--model_dir` `--output_dir` data, checkpoint and output locations | **Operational inference on any date.** Reads the reprojected full-domain fields and slices all 18 patches on the fly — touches no `patch_index.csv`, split CSV, or pre-extracted patch tile, so it runs on dates the training pipeline has never seen. | `python predict_full_domain.py --mode mtg_opera_mtgmr_rainfall --date 2026-06-30`<br>`python predict_full_domain.py --mode mtg_opera_mtgmr_rainfall --date 2026-06-30 --hour 14`<br>`python predict_full_domain.py --mode mtg_lightning_opera_occurrence --date 2026-06-30 --time 14:30`<br>`python predict_full_domain.py --mode mtg_lightning_opera_occurrence --date 2026-06-30 --validation_summary validation/lightning_2025_05_summary.json`<br>`python predict_full_domain.py --mode mtg_opera_mtgmr_rainfall --date 2026-06-30 --start-time 12:00 --end-time 15:45 --save-npy` |
| **5** | `visualize_gt_vs_pred.py` | `--csv PATH` split CSV listing candidate references; required, its rows name the patch files the ground truth is built from · `--weights best\|latest` the final save or the per-epoch checkpoint (`_latest` suffix on the output folder) · `--mode` model variant to visualise; required · `--top_n N` how many references to plot, default 5 · `--baseline` visualise the SepConv-ens baseline of `--period` instead of a model, its class map through the same rows · `--pick csi\|active` choose them by the validation run in `--validation_summary` instead of by patch count: the `--top_n` best samples by mean CSI over leads, or the `--top_n` with the most ground-truth-active pixels over the leads; each must also be a row of `--csv`; outputs are prefixed `csi_top<NN>_` or `active_top<NN>_` · `--cvae` the conditional-VAE fine-tuned head · `--finetuned` \| `--kd` visualise Swin head or KD student · `--eval_results PATH` evaluation JSON supplying the decision threshold · `--threshold` fixed threshold overriding the evaluation JSON · `--no_zoom` skip the per-patch zoom figure · `--no_aggregate_graphs` skip the rainfall aggregate plots · `--stride` Hann overlap stride for lightning inference · `--lightning_low_threshold` `--lightning_high_threshold` hysteresis pair, lightning head · `--rainfall_low_threshold` `--rainfall_high_threshold` hysteresis pair, rainfall head · `--validation_summary PATH` load tuned per-lead hysteresis thresholds · `--batch_size` samples per inference step · `--data_root` `--model_dir` `--output_dir` dataset, checkpoint and output locations | **Training-scope visualiser.** Ranks references in a split by qualifying-patch count and renders GT beside predictions, plus a zoom on the patch with the most GT activity. Hysteresis knobs mirror `predict_full_domain.py` so both render identical post-processing. | `python visualize_gt_vs_pred.py --csv our_data/test_data_dbscan.csv --mode mtg_opera_mtgmr_rainfall`<br>`python visualize_gt_vs_pred.py --csv our_data/validation_data_dbscan.csv --mode mtg_lightning_opera_occurrence --top_n 3`<br>`python visualize_gt_vs_pred.py --csv our_data/test_data_dbscan.csv --mode mtg_opera_occurrence --kd --no_zoom` |
| **6** | `generate_report.py` | `--year` `--month 1-12` reporting period; both required · `--track rainfall\|lightning\|both` which tracks the report covers · `--language ro\|en` output language; `en` skips translation · `--bilingual` render Romanian and English side by side · `--model TAG` Ollama model tag used for generation · `--temperature` sampling temperature; 0.0 collapses Gemma · `--seed` Ollama seed for reproducible generation · `--max_tokens` hard cap on tokens per call · `--refresh_cache` `--no_cache` rebuild or bypass the translation cache · `--skip_pdf` run generation without rendering the PDF · `--pred_coupling` couple cells from predictions, not ground truth, with the models named by `--period`, `--weights best\|latest`, `--rainfall_variant base\|finetuned\|cvae` (cvae composes the members of the rainfall summary), `--lightning_variant base\|finetuned\|kd`, at the thresholds both summaries tuned · `--split train\|validation\|test` `--rainfall_threshold_mmh` read split-scope validation runs restricted to the month · `--top_n N` the N references with the most coupled pixels each get a coupling figure (default 5) · `--coupling_only` stop after those figures, no text generation and no PDF · `--validation_dir` `--assets_dir` `--output` inputs, banner assets, PDF destination · `--data_root` `--model_dir` dataset and checkpoint locations | Builds a PDF from `validate_predictions.py` outputs with commentary from a local Ollama LLM. `--language en` skips the Romanian translation phase and halves the LLM calls. `--track both` requires both tracks' extraction outputs on disk. | `python generate_report.py --year 2025 --month 5`<br>`python generate_report.py --year 2025 --month 5 --language en`<br>`python generate_report.py --year 2025 --month 5 --track lightning --bilingual`<br>`python generate_report.py --year 2025 --month 5 --skip_pdf --no_cache` |
...
| **9** | `data_statistics.py` | `--split train\|validation\|test` which split to summarise · `--csv PATH` explicit CSV overriding the split default · `--data_root PATH` root holding the per-split CSVs | Four dataset diagnostic panels: active patches per hour of day, spatial heatmap, simultaneously-active patches, patch survival. | `python data_statistics.py`<br>`python data_statistics.py --split test` |
| **10** | `compare_models.py` | `--track rainfall\|lightning` which validation/evaluation family is compared; required · `--eval_root` `--validation_dir` `--output_dir` locations (default `evaluation/`, `validation/`, `comparison/`) · `--include_baseline` add the SepConv-ens baseline rows and curves · `--models TAG…` restrict to these artefact tags, each naming the exact saved state (a plain tag the final save, a `_latest` tag the checkpoint run) · `--weights best\|latest\|any` which saved state of each model is read: the final saves, the per-epoch checkpoint runs (`_latest` artefacts), or any (default: `best` when it exists, else `latest`); the `_latest` suffix never splits a model in two · `--label TAG=NAME` display names · `--hit_levels 50 60 70 80 90` hit percentages on the per-sample plots · `--seasons NAME=M,M…` season definition (default DJF / MAM / JJA / SON) · `--split train\|validation\|test` `--year` `--month` which validation runs feed the hit-level figures: the split's, one month's, or (default) every whole-month run pooled · `--rainfall_threshold_mmh T` which selection threshold's runs, needed only when several are on disk | Every model of one track side by side. Reads each model's `evaluation_results.json` → one `metrics_per_leadtime_<metric>.png` per metric with all models plus a grid, and a paper-style table (`comparison_table.csv/.md/.tex`, best value in bold). Reads each model's validation `_samples.csv` → the share of samples whose hit percentage (GT-active pixels detected on the post-processed map at the tuned HIGH; per-sample POD for lightning) clears 50…90 %, per lead, per season and over the whole window (`hit_levels_<scope>/hit_levels.png`, `hit_levels.csv`, one folder per scope). Untagged legacy validation files are skipped. See [`comparison/`](#compare_modelspy--comparisontrack). | `python compare_models.py --track rainfall --split test --include_baseline`<br>`python compare_models.py --track rainfall --year 2026 --month 6`<br>`python compare_models.py --track lightning --split test` |
...
| **12** | `our_data/lightning_data/visualize_lightning_stats.py` | `--csv PATH` lightning activity index CSV to plot · `--output_dir PATH` destination for the chart | Lightning activity by time of day — for each HH:MM, how many days carry a stroke — from step 2c's activity index. The one view only the per-(date, HH:MM) record can give. Plots only — reads no model. | `python our_data/lightning_data/visualize_lightning_stats.py` |
...
<p align="center"><img src="assets/table3_architecture_defaults.png" width="100%" alt="Table 3 — Architecture & training defaults"/></p>
...
| Past / future timesteps | read from the dataset — `w34`: 3 / 4 | all | `sequence_meta_<source>[_<period>].json` |
| **Base training** (`[defaults]`) ||||
| Optimizer | `Adam(lr=1e-3)` | base stage | — |
| Loss | `WeightedFocalLoss(gamma=2.0)`, prior from `lightning_fraction_<source>[_<period>].json` (~1 % positive pixels) | lightning | — |
...
| Epochs / batch size | `30` / `32` | all | `[defaults]` |
...
| Patience / min delta | `8` / `1e-5` | all | `[early_stopping]` |
...
| **Fine-tune head** (`[finetune]`, `--stage finetune`/`both`, `--head`) ||||
| Head | `swin_unet`: two-level Swin-UNet correction on the frozen backbone, `logits = s_l · L_b + head(x)`, heads zero-initialised, `s_l = 1`; `swin_unet_cvae` adds the conditional VAE (rain); `swin_legacy` is the earlier single-level head | finetune | `[finetune] head`, `--head` |
| Levels | level 1 at 1 km, `d = 64`, 4 heads; level 2 at 2 km, `d = 128`, 8 heads; 2 blocks per level, 8 × 8 windows, relative position bias and shift mask, MLP ratio 2, stochastic depth 0.1 | finetune | `[finetune]` |
| Stems | MR stem (last 2 MR frames) joins level 1; target stem (last 2 frames of the target field at 1 km) joins the output, width 16; patch expand to 32 channels | finetune | `[finetune]` |
| Loss, lightning | BCE with `pos_weight = min(N_neg/N_pos, 30)`; `lightning_gamma = 2` as the focal fallback | finetune | `[finetune]` |
| Loss, rain deterministic | weighted focal CE of `[radar_loss]` + `0.2 ·` expected-class L1 | finetune | `[finetune]` |
| Loss, rain cVAE | class-weighted CE (no focal) + `beta · mean_i max(KL_i, free bits)`, `beta = 0.02` after a 2-epoch warm-up, free bits `0.2` nats; latent `16² × 32`, gated FiLM on level 2 | finetune | `[finetune]` |
| Optimizer | `AdamW`, `lr 2e-4`, `weight_decay 0.05` off the norms, biases, position tables and `s_l`; cosine with a 1-epoch warm-up, gradient clipping 1.0 | finetune | `[finetune]` |
| Epochs / early stopping | `18`, early stopping on `val_csi_mid` (mean CSI of t+30 and t+45), patience 4, best weights restored | finetune | `[finetune]` |
| Backbone | frozen — only the head trains; step 0 reproduces the backbone exactly (checked at start) | finetune | — |
| **Knowledge distillation** (`train_lightning_kd.py`) ||||
| Loss | `α·KL(teacher‖student)·T² + (1−α)·WeightedFocalLoss`, the KL weighted by the teacher probability + 0.01 | KD student | `--kd_soft_weight` / `--kd_weight_floor` |
| Alpha / temperature | `0.7` / `2.0` | KD student | `--kd_alpha` / `--kd_temperature` |
| Optimizer / LR | `Adam`, `cosine_warmup` from `[lr_schedule]` (1e-3 → 1e-6, 3 warmup epochs), same as the base models | KD student | `--config`, `--learning_rate` overrides `initial_lr` |
| Epochs / batch / patience | `50` / `8` / `10` | KD student | CLI |
| **SepConv baseline** (`sepconv_ensemble_training.py`, `[sepconv]`) ||||
| Architecture | 3 independent `SeparableConv2D` models, one per lead (`1`, `3`, `5` steps), 4 input frames each; kernel `5×5`; ~100k parameters | baseline | — |
| Optimizer | `Adam(lr=1e-3, amsgrad=True)`, halve on plateau (`lr_patience = 5`) | baseline | `[sepconv]`, LR from `[lr_schedule]` |
| Epochs / batch / patience | `30` / `32` / `8` — inherited from `[defaults]` and `[early_stopping]` unless set in `[sepconv]` | baseline | `[sepconv]` |
| Loss / output | weighted MSE in `log_zscore` space, weights from the measured class frequencies; linear output, denormalised to mm/h and binned at `RAINFALL_CLASS_EDGES` at evaluation | baseline | — |
...
<p align="center"><img src="assets/stage0.png" width="100%" alt="Stage 0 — master cadence"/></p>
...
<p align="center"><img src="assets/stage1.png" width="100%" alt="Stage 1 — acquisition"/></p>
...
| `our_data/lightning_data/linet_export.py` | `<out>/kml_data/<date>/<date>.kml` | `read_kml_version2` | Raw LINET stroke exports, one KML per day. `--end` is inclusive, like everywhere else in the pipeline. |

</details>

### Stage 2 — onto the Romania grid

<p align="center"><img src="assets/stage2.png" width="100%" alt="Stage 2 — onto the Romania grid"/></p>
...
| `our_data/lightning_data/summarize_lightning_data.py` | `our_data/lightning_data/`: `lightning_summary.csv` · `lightning_missing_timesteps.json` · `lightning_active_steps.csv` · `lightning_coverage.png` (`--chart`) | `intersect` (missing JSON) · `lightning_fraction` and `visualize_lightning_stats` (activity index) · chart is **terminal** | The intersection gates lightning on **presence**, like MTG and OPERA: a frame of zeros is an observation. A quiet day gets its zero frames from `read_kml_version2`, so it is a day, not an outage. `lightning_active_steps.csv` remains the activity index behind `visualize_lightning_stats`' diurnal chart. |
...
<p align="center"><img src="assets/stage4.png" width="100%" alt="Stage 4 — sample selection"/></p>
...
| Script | Files written | Consumed by | What it holds & why |
|---|---|---|---|
| `identify_patches.py` | `our_data/patch_index/patch_index.csv` · `patch_index.json` · `plots/dbscan_<rule>_patch_selection/<date>.gif` · `plots/patch_highlight/<date>.gif` · `plots/nc/patches_<date>_<HHMM>.nc` | `extract_patch_seq_for_datasets` · `extract_patches` · `data_statistics` · plots + `.nc` are **terminal** | DBSCAN over OPERA rain rate (threshold and rule recorded in `patch_index.json`; eps 5, min_samples 20) flags which of the 18 patches are convectively active per timestep. **Selects patches, not pixels** — every pixel of a chosen patch is used, dry ones included, which is why the weighted losses exist. Two rules turn clusters into tiles, recorded in the index as `selection_rule`: **`pixel`** selects a tile that holds ≥ 1 cluster pixel (every index so far); **`box`** centres a 256 × 256 box on each cluster centroid and selects a tile that meets the union of those boxes, so every cell keeps 128 px of context on each side and an edge cell brings its dry neighbour in with it. One index serves every period, and its row order defines the patch axis of the saved arrays. Diagnostics cost ~28 MB each; `--purge_plots` clears them. |
...
<p align="center"><img src="assets/stage5.png" width="100%" alt="Stage 5 — statistics & class priors"/></p>
...
| `lightning_fraction.py` | `our_data/lightning_fraction_<source>[_<period>].json` | `train_models` (occurrence modes) | Fraction of non-zero pixels in the occurrence maps — the focal-loss `ones_fraction`. Both priors take `--period` so scope and filename come from one tag. |

</details>

### Stage 6 — datasets

<p align="center"><img src="assets/stage6.png" width="100%" alt="Stage 6 — datasets"/></p>
...
<p align="center"><img src="assets/stage7.png" width="100%" alt="Stage 7 — trained models"/></p>
...
| Script | Files written | Consumed by | What it holds & why |
|---|---|---|---|
| `train_models.py` | `models/coalition_<run_tag>[_finetuned\|_finetuned_cvae].keras` · `models/history_<run_tag>[_finetuned\|_finetuned_cvae].json` (written after every epoch, `"complete": false` until the run ends, so an interrupted run keeps its curves) · `models/coalition_<run_tag>.meta.json` · `models/checkpoints/<run_tag>_latest.keras` · `_latest.json` | `predict_full_domain` · `validate_predictions` · `evaluate_coalition` · `visualize_gt_vs_pred` · `build_patch_ensemble` · `train_lightning_kd` | Base and Swin-finetuned weights. The history JSON records mode, source, stage, label type, epochs and wall time — enough to tell two runs apart later. The `.meta.json` sidecar pins **the period a model was trained on**, which is what the feature-extractor leakage gate checks before reusing frozen weights. |
| `sepconv_ensemble_training.py` | `models/sepconv_<run_tag>_bm{1,3,5}.keras` · `models/history_sepconv_ensemble_<mode>.json` | `sepconv_predict` · `evaluate_sepconv_ensemble` | The three SepConv-ens base models, predicting 1 / 3 / 5 steps past the end of their own window (which lands them on t+1, t+2/t+3 and t+4). Combined at inference by the composition table, never retrained together — which is why they are three files, not one. |
...
<p align="center"><img src="assets/stage8.png" width="100%" alt="Stage 8 — inference, validation & reporting"/></p>
...
| `validate_predictions.py` | `validation/<stem>_summary.json` with `<stem>` = `<track>_[<split>_][<yyyy>_<mm>_]thr<T>mmh_<tag>` · figures in `validation/<stem>/` · `..._samples.csv` · `..._metrics.png` · `..._coverage.png` · `..._hmf.png` · `..._hmf_percentiles.png` · `..._tuning.png` · `..._<date>_<HHMM>.png` | `generate_report` · `build_patch_ensemble` · `bundle_eval_scores` | Per-lead POD / FAR / CSI with the hysteresis HIGH tuned by maximising aggregate CSI, plus a `per_patch` block. That block is what the ensemble selector reads to decide **which member wins each patch**. `--baseline` writes the same files for SepConv-ens under the `sepconv_<run_tag>` tag. |
| `compare_models.py` | `comparison/<track>/metrics_per_leadtime.png` · `comparison_table.{csv,tex,png}` · `hit_levels_<scope>/hit_levels.png` · `hit_levels_<scope>/hit_levels.csv` · `comparison_summary.json` | — (deliverable) | All models of a track on one axis: per-lead metric curves, the paper table, and the share of samples above each hit level per lead / season / whole window. |
...
**Picked timesteps:** `--pick csi --validation_summary <summary.json>` runs the `--top_n` best samples (default 5) by mean CSI over leads of that validation run; `--pick active` the `--top_n` with the most ground-truth-active pixels over the leads (rain ≥ 10 mm/h or lightning occurrence, from the `gt_active_px_t+*` columns of the samples CSV, or from the ground-truth files for an older run). The two sets are prefixed `csi_top<NN>_` and `active_top<NN>_`, so they do not overwrite each other.
...
### `visualize_gt_vs_pred.py` → `visualize_gt_vs_pred_plots/visualize_gt_vs_pred_<run_tag>[…]/`

- **3-row full-domain figure** (`ts<NN>_<date>_<HHMM>.png`; `csi_top<NN>_` or `active_top<NN>_` instead of `ts<NN>_` with `--pick csi` or `--pick active`) — Row 1 GT, Row 2 raw prediction, Row 3 post-processing overlap. Green/red patch numbering shows which patches DBSCAN selected vs padded.
...
The summary JSON and the samples CSV are written flat in `validation/`, which is where `compare_models`, `generate_report` and `build_patch_ensemble` read them. Every PNG of a run, the figures and the per-date overlays, goes into a folder named after the run's stem, `validation/<track>_<scope>_<tag>/`, each file still prefixed with the stem.

Two coverage metrics per (sample, lead): **`iou_mask`** (IoU of the binary masks at the selected threshold — structure only) and **`class_wt`** (per-class weighted overlap macro-averaged across the 5 classes — semantic-aware). Aggregate FAR / POD / CSI are computed per lead on the binary ≥10 mm/h event.

| File | Contents |
|---|---|
| `<track>_<scope>_<tag>_samples.csv` | One row per sample; both metrics × each lead time, `csi_t+<k>` per lead on the post-processed map at the tuned HIGH, `hits_pct` / `misses_pct` / `false_alarms_pct` per lead on the post-processed map at the tuned HIGH (hits and misses over the GT-active pixels, false alarms over the predicted-active pixels; empty where the denominator is zero), |
| `<track>_<scope>_<tag>_summary.json` | `split`, `model`, totals, counts above `--high_coverage_pct` per lead × metric, FAR/POD/CSI per lead, initial selection list, tuned `post_processing` thresholds. |
| `<track>_<scope>_<tag>_metrics.png` | Grouped FAR / POD / CSI bars, one bar per lead. Both tracks. |
| `<track>_<scope>_<tag>_coverage.png` | Per-sample coverage scatter, IoU against class-weighted overlap, marker per lead, red lines at 50 %, gray dotted at the coverage grade. Rainfall only. |
| `<track>_<scope>_<tag>_hmf.png` | Three panels, hits % / misses % / false alarms %, each a per-sample scatter in chronological order with a marker per lead and a red line at 50 %; the x axis names the first, median and last sample. Both tracks (lightning's come from its per-sample POD and FAR). |
...
| `<track>_<scope>_<tag>_members.png` | cVAE rain head only: one panel per member with its per-patch CSI on the validation split at the tuned thresholds, the patches it won in green; the winner per patch is what the scope was scored with and what the visualiser and the inference compose. |
| `<track>_<scope>_<tag>_hysteresis.png` | Three plots of the hit / miss / false-alarm rates (hits and misses over the GT-active pixels, false alarms over the predicted-active pixels). Per lead, the rates of the raw decision (hatched) and of the post-processed one (solid). Per lead, the difference post minus raw of each rate, in percentage points. For the leads after the first, the change of each rate against the first lead, raw and post-processed. Raw is the argmax class map on the rainfall track and p ≥ LOW on the lightning track, recorded as `metrics_per_lead_raw`. Both tracks. |
| `<track>_<scope>_<tag>_tuning_low.png` · `_tuning_high.png` | Rainfall: phase 1, pooled CSI against the plain threshold per lead with a dashed line at the chosen LOW and its value and CSI in the legend; phase 2, one panel per lead with a bar per (LOW, HIGH) window, the best one full-colour with its CSI on it. Both on the validation-split samples; one lead only with `--tune_leads first`. Absent for the baseline. |
| `<track>_<scope>_<tag>_tuning.png` | Lightning: pooled CSI on the validation-split samples against the swept HIGH, one line per lead, a dashed line at each lead's winner with its value and CSI in the legend. |
...
`<scope>` says which samples were scored: `<yyyy>_<mm>` for a calendar month, `test` (or `train`, `validation`) for the reference timesteps of that dataset split, `test_<yyyy>_<mm>` for the split restricted to a month, always followed by `thr<T>mmh`, the selection threshold (`--rainfall_threshold_mmh`), so runs at different thresholds never overwrite each other: `rainfall_test_thr8mmh_<tag>_*`. `<tag>` is the validated model's artifact tag (`<mode>_<source>_<period>`, with `_finetuned` / `_kd` when applicable, or `sepconv_<run_tag>` with `--baseline`). Every figure is drawn from the summary JSON and the samples CSV, at the end of the run and again on demand with `python validate_predictions.py --plots <summary.json>`; no model or data is touched. The visualisation mode (`--date`) reads the JSON of the same scope, so pass the same `--split` / `--year --month` it was written with. `generate_report` takes `--rainfall_tag` / `--lightning_tag` when more than one model was validated for the month.

### `compare_models.py` → `comparison/<track>/`

One figure set per track; every model whose `evaluation/eval_<tag>/evaluation_results.json` (and, for the hit-level plots, whose tagged validation `_samples.csv`) exists is drawn, the baseline included with `--include_baseline`.

> [!WARNING]
> No lightning baseline exists yet. `--include_baseline` therefore adds > rows and curves on the rainfall track only; on the lightning track it > changes nothing until a lightning baseline is trained and validated.

| File | Contents |
|---|---|
| `metrics_per_leadtime_<metric>.png` | One line per model over the lead times, one figure per metric; `metrics_per_leadtime.png` is the grid of all of them. |
| `comparison_table.csv` · `.tex` · `.png` | Paper-style table, one row per model, per-lead and aggregate columns; best value per column in bold (FAR lower is better); the CSV for machines, the LaTeX for the paper, the PNG as the asset of the README. |
| `hit_levels_<scope>/hit_levels.png` | Rows are the four seasons plus the whole window, columns the lead times; each panel is the share of samples whose hit percentage (rainfall: `hits_pct`, GT-active pixels detected on the post-processed map at the tuned HIGH; lightning: per-sample POD) is at least 50 / 60 / 70 / 80 / 90 % (`--hit_levels`), one line per model. `<scope>` is `test` (`--split test`), `<yyyy>_<mm>` (`--year --month`), `test_<yyyy>_<mm>` (both) or `all_months` (neither: every whole-month validation run pooled), followed by `thr<T>mmh`, the selection threshold of the runs used. Seasons come from each sample's own date. |
| `hit_levels_<scope>/hit_levels.csv` | The numbers behind those figures. |
| `comparison_summary.json` | Which models, files and thresholds went in. |
...
| Label | Binary lightning occurrence at each future step | *(same)* |
...
|---|---|---|---|---|
| `DBSCAN_THRESHOLD` | `10` mm/h module default; the master `patch_index.json` records the value it was built with (8 mm/h today), which is the default once it exists | `--threshold` | Rain-rate cut for training-patch selection; pixels strictly above it are clustered. | Lower → weaker events enter training. Higher → smaller, more selective training set. Changing it rebuilds a different index and invalidates the patch pool. |
| `selection_rule` | `pixel` | `--rule pixel\|box` | How clusters become tiles. `pixel`: a tile holding ≥ 1 cluster pixel. `box`: a tile meeting the union of 256 × 256 boxes centred on the cluster centroids. Recorded in `patch_index.json`; the recorded rule is the default once an index exists. | `box` selects more tiles — the dry neighbours of cells near a tile edge — and guarantees 128 px of context around every cell. Changing the rule rebuilds a different index and invalidates the patch pool. |
...
| `DEFAULT_RAIN_LOW` / `DEFAULT_RAIN_HIGH` | `0.20` / `0.25` | `--rainfall_low_threshold` / `--rainfall_high_threshold` | Hysteresis on `p(argmax)` when the argmax is a rainy class; the fallback when no `--validation_summary` is given. Much lower than the lightning pair because probability is split across 5 classes: rainy pixels of the f34 model carry 0.24–0.46 (the former 0.35 / 0.55 left the map empty). Rejected pixels drop to class 0. | Lower → more marginal rainy pixels survive. Higher → tighter blobs. |
| window / reach | `0.02` / `0.10` | `--rainfall_window` / `--rainfall_reach` | The phase-2 (LOW, HIGH) windows of the rainfall tuning: width of a window, and how far they slide on each side of the phase-1 LOW. | Wider windows → HIGH further above LOW, coarser choice. Longer reach → more candidates, longer tuning. |
...
`[0,15,30,45]`. MTG stays at its native 10 min with the `[0,10,30,40]`
filter, so it sits up to 5 minutes off the label.
...
├── visualize_gt_vs_pred_plots/  # visualize_gt_vs_pred output
