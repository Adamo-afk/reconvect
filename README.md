...
| **10** | `compare_models.py` | `--track rainfall\|lightning` which validation/evaluation family is compared; required · `--eval_root` `--validation_dir` `--output_dir` locations (default `evaluation/`, `validation/`, `comparison/`) · `--include_baseline` add the SepConv-ens baseline rows and curves · `--models TAG…` restrict to some artifact tags · `--label TAG=NAME` display names · `--hit_levels 50 60 70 80 90` hit percentages on the per-sample plots · `--seasons NAME=M,M…` season definition (default DJF / MAM / JJA / SON) | Every model of one track side by side. Reads each model's `evaluation_results.json` → one `metrics_per_leadtime_<metric>.png` per metric with all models plus a grid, and a paper-style table (`comparison_table.csv/.md/.tex`, best value in bold). Reads each model's validation `_samples.csv` → for each rain-rate threshold of the sweep, the share of samples whose hit percentage clears 50…90 %, per lead, per season and over the whole window (`hit_levels_ge<T>.png`, `hit_levels.csv`). Untagged legacy validation files are skipped. See [`comparison/`](#compare_modelspy--comparisontrack). | `python compare_models.py --track rainfall --include_baseline`<br>`python compare_models.py --track lightning` |
...
| File | Contents |
|---|---|
| `<track>_<year>_<month>_<tag>_samples.csv` | One row per sample; both metrics × each lead time, plus `hit_pct_t+<k>_ge<T>` — the percentage of GT pixels at or above threshold T that the prediction also puts at or above T, for every T of the sweep (base, base + step, … base × factor). |
| `<track>_<year>_<month>_<tag>_summary.json` | `model`, `hit_thresholds_mmh`, `hit_pct_pooled_per_lead`, totals, counts above `--high_coverage_pct` per lead × metric, FAR/POD/CSI per lead, initial selection list, tuned `post_processing` thresholds. |
| `<track>_<year>_<month>_<tag>_metrics.png` | Left: grouped FAR/POD/CSI bars per lead. Right: per-sample coverage scatter (IoU vs class-weighted, marker per lead). |
| `<track>_<year>_<month>_<tag>_<date>_<HHMM>_<lead>.png` | Visualisation mode. Left: structure overlay (red = GT class == Pred class and both ≥10 mm/h). Right: 256 × 256 zoom into the most GT-active patch — red matched, blue misses, orange false alarms. |
...
`<tag>` is the validated model's artifact tag (`<mode>_<source>_<period>`, with `_finetuned` / `_kd` when applicable, or `sepconv_<run_tag>` with `--baseline`). `generate_report` takes `--rainfall_tag` / `--lightning_tag` when more than one model was validated for the month.
...
| `hit_levels_ge<T>.png` | For threshold T of the sweep: rows are the four seasons plus the whole window, columns the lead times, bars the share of samples whose hit percentage is at least 50 / 60 / 70 / 80 / 90 % (`--hit_levels`), one bar group per model. |
| `hit_levels.csv` | The numbers behind those figures. |
