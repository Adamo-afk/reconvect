...
| Phase | What | Figure |
|---|---|---|
| 1 | LOW per lead: a plain threshold on p(argmax) of the rainy-argmax pixels, swept 0.01 … 0.99 through probability histograms; the best pooled CSI wins. `--rainfall_low_threshold` skips this and fixes one LOW for every lead. | `<stem>_tuning_low.png` — CSI against the threshold, one line per lead, star at the chosen LOW |
| 2 | (LOW, HIGH) per lead: windows of `--rainfall_window` (default 0.02) slid on both sides of the phase-1 LOW out to `--rainfall_reach` (default 0.10). For 0.35: (0.33, 0.35), (0.31, 0.33), … and (0.35, 0.37), (0.37, 0.39), …; every window is one hysteresis run; the best pooled CSI wins. | `<stem>_tuning_high.png` — one panel per lead, a bar per window, star at the chosen pair |
