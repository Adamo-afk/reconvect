...
| `compare_models.py` | `comparison/<track>/metrics_per_leadtime_<metric>.png` · `comparison_table.{csv,tex,png}` · `hit_levels_<scope>/hit_levels.png` · `hit_levels_<scope>/hit_levels.csv` · `comparison_summary.json` | — (deliverable) | All models of a track on one axis: per-lead metric curves, the paper table, and the share of samples above each hit level per lead / season / whole window. |
