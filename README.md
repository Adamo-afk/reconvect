...
**Picked timesteps:** `--pick csi --validation_summary <summary.json>` runs the `--top_n` best samples (default 5) by mean CSI over leads of that validation run, under the same file names.
...
- **3-row full-domain figure** (`ts<NN>_<date>_<HHMM>.png`; `csi_top<NN>_` instead of `ts<NN>_` with `--pick csi`) — Row 1 GT, Row 2 raw prediction, Row 3 post-processing overlap. Green/red patch numbering shows which patches DBSCAN selected vs padded.
