...
Standard modes: `msg_lightning`, `msg_radar`, `mtg_lightning`, `mtg_radar`.
...
```bash
python train_models.py --mode msg_lightning --epochs 10 --batch_size 32
python train_models.py --mode msg_radar --epochs 10 --batch_size 32
python train_models.py --mode mtg_lightning --epochs 10
python train_models.py --mode mtg_radar --epochs 10
python train_models.py --mode msg_lightning --epochs 1 --batch_size 8   # quick test
...
Both radar composites and MTG FCI data are acquired at native 10-minute cadence (:00, :10, :20, :30, :40, :50). The COALITION-4 pipeline requires a ~15-minute cadence. Rather than synthesizing missing timestamps via optical flow interpolation (which introduces additional errors into training samples), the arrange scripts directly select the nearest native timestamps:

| Pipeline target | Selected native | Offset |
|---|---|---|
| :00 | :00 | 0 min |
| :15 | :10 | -5 min |
| :30 | :30 | 0 min |
...
This is the default behavior of both `radar_arrange.py` and `pipeline_msg_mtg.py` (both filter to minutes `00, 10, 30, 50` by default). Use `--timesteps all` in either script to keep every native timestep.
